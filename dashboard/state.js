window.PARADISE_STATE = {
  "generated": "2026-09-16T07:34:00.487Z",
  "pipeline": [
    {
      "id": "discover",
  ...
  "graph": {
    ...
    "nodes": [
      ...
      {
        "id": "fix-all-siblings-of-the-same-flaw",
        "type": "lesson",
        "label": "同じ関数の中で同じ病を片側だけ直すな",
        "degree": 0
      },
      {
        "id": "gate-neutralized-by-or-true",
        "type": "lesson",
        "label": "|| true を付けた門は門ではない",
        "degree": 0
      },
      {
        "id": "epoch-marker-needs-a-timestamp",
        "type": "lesson",
        "label": "印の有無だけを見る門は印を消せば消える",
        "degree": 0
      },
      {
        "id": "abandoned-run-must-ring",
        "type": "lesson",
        "label": "見捨てられた走行は機構が名指しする — マージは環の完了ではない",
        "degree": 0
      },
      {
        "id": "critic-exemption-self-declaration",
        "type": "lesson",
        "label": "門は免除の自己申告を信じない — 空の .paradise-source 1個で創造物の掟3法が素通りした。免除の資格は住所(workspace.js)が裏付ける。免除も却下も必ず名指しで出力に残す (第54条)",
        "degree": 0
      },
      {
        "id": "ring-not-closed-by-merge",
        "type": "lesson",
        "label": "マージは環の完了ではない — 実装が main へ入っても走行帳が 5/11 のまま週単位で放置された実例がある。域を健全と断ずる前に node graph/conclave.js audit で実測せよ|check",
        "degree": 0
      },
      {
        "id": "mutation-author-is-blind-to-own-net",
        "type": "lesson",
        "label": "自分で打った変異が全部鳴っても網は証明されない",
        "degree": 0
      },
      {
        "id": "repair-opens-a-new-surface",
        "type": "lesson",
        "label": "修理は次の攻撃面を開く — 差分が塞いだ穴でなく差分が開いた面を撃て",
        "degree": 0
      },
      {
        "id": "idempotent-key-can-be-preempted",
        "type": "lesson",
        "label": "決定的な冪等鍵は先取りできる — 冪等化は先回り毒の面を必ず開く",
        "degree": 0
      },
      {
        "id": "rework-four-times-is-a-design-debt",
        "type": "lesson",
        "label": "四度の差し戻しは実装の弱さでなく設計の空白である",
        "degree": 0
      },
      {
        "id": "gate-filter",
        "type": "reform",
        "label": "門の絞り込みの口 — 自己診断を名指しで走らせる。449本を20.7秒(前: 449本を358.8秒)。第56条を生んだ",
        "degree": 0
      },
      {
        "id": "selection-must-run",
        "type": "lesson",
        "label": "選んだ門を走らせない走行は測定ではない。matched=green+red の恒等式を、選別を行う関数の外側で検めよ|check",
        "degree": 0
      },
      {
        "id": "warden-outside-selection",
        "type": "lesson",
        "label": "門番が絞り込みの対象なら門番ではない。機構を見張る門は、その機構が選び落とせない位置に置け|check",
        "degree": 0
      },
      {
        "id": "artifact-name-is-machine-contract",
        "type": "lesson",
        "label": "相の名(discover)と成果物の名(findings.md)は別物。critic は名前で探すので、成果物の名は機械に合わせよ|findings",
        "degree": 0
      },
      {
        "id": "child-stdout-exit-truncation",
        "type": "lesson",
        "label": "子プロセスが console.log の直後に process.exit() を呼ぶと、POSIX ではパイプ緩衝(64KiB)を越えた stdout が掃き出される前に死に、親は status 0 のまま切れた JSON を読む。Windows は同期書き込みなので再現せず「手元は緑、CI だけ赤」になる。終了コードは process.exitCode で名乗り exit を呼ぶな。また限度超過を「JSON が壊れている」と名乗ってはならない(第34条)",
        "degree": 0
      },
      {
        "id": "gate-red-needs-a-mouth-not-a-wider-gate",
        "type": "lesson",
        "label": "門が正しく赤いのに現実は健全なとき、真因は「機構に語彙が無い」ことでありうる。境を伸ばす/例外を足す前に、engine に足りない口(記録の語)を探せ。口には濫用を禁じる錬(実在・非空・二度指し禁止・状態不変・註釈の実質)を掛け、逆の門(口が無い場合は今まで通り赤い)で固定する|check",
        "degree": 0
      },
      {
        "id": "both-branches-of-a-rule",
        "type": "lesson",
        "label": "判定規則が二枝を持つなら、片枝の証明は他枝について何も語らない。弱い名に二条件を課して誤着を塞いだ走行が、強い名 26 語を無条件で通す口をそのまま残し、世間の願い 21/21 を reform へ攫った(main では 0/21)。片側を直したら必ず反対側を同じ探針で撃て",
        "degree": 0
      },
      {
        "id": "regression-needs-both-heads",
        "type": "lesson",
        "label": "回帰か否かは HEAD だけでは判じられない。同じ探針を main と HEAD の両方で撃って初めて本走行の産物と断定できる。第38条の前後比較を成果物の数にしか使わず、振る舞いに使わなかった走行は、自分が作った誤着を仕様と見誤る",
        "degree": 0
      },
      {
        "id": "corpus-blind-to-its-own-half",
        "type": "lesson",
        "label": "門のコーパスが規則の片枝しか含まないなら、門は永久に緑のまま面の半分を素通しする。AC-31 の 24 件は全て弱い名で作られ、強い名は 0 件だった。コーパスは規則の枝ごとに数えて示せ。故障注入の変異も同じ — 全変異が片枝を狙うなら、他枝について何も証明していない",
        "degree": 0
      },
      {
        "id": "filter-in-a-gate-is-a-silent-exemption",
        "type": "lesson",
        "label": "網羅を主張する門の中の filter は沈黙の免除である。graph/*.js の網羅を撃つ門が n.length >= 4 で kg を黙って落とし、kg が語彙表に無いまま門は永久に緑だった。門が測る対象と門が主張する範囲がずれていないか、除外した件数を必ず出力に名乗らせよ",
        "degree": 0
      },
      {
        "id": "strength-of-evidence-sets-the-gate",
        "type": "lesson",
        "label": "判定の枝が複数の証拠を持つとき、証拠の強さが許す条件の広さを決める。強い証拠(engine の強い固有名)に弱い証拠と同じ条件を機械的に写すと片方が壊れる — 強い名に BUILD_RE だけを課すと『conclave の毒を除く』が死ぬ。ゆえに強い名には BUILD_RE ∪ MEND_RE を課した。check",
        "degree": 0
      },
      {
        "id": "mutate-the-gate-not-only-the-impl",
        "type": "lesson",
        "label": "故障注入が実装側だけを狙うなら、門のコーパスの穴は永久に見つからない。門(コーパス・表との照合)の側を壊す変異を必ず一つ以上持て。F-4 は変異 3 件が全て実装側だったために、29 件のコーパスが守る面の半分を撃っていないことを誰も見なかった。check",
        "degree": 0
      },
      {
        "id": "corpus-machine-checked-against-table",
        "type": "lesson",
        "label": "『表の全語を撃っている』という主張は、表とコーパスを機械で照合して初めて真になる。人が足し忘れれば F-4 は再演する。ENGINE_NAMES_STRONG の全語 ⇔ コーパスの過不足を両方向で assert せよ。check",
        "degree": 0
      },
      {
        "id": "restore-path-must-be-verified",
        "type": "lesson",
        "label": "故障注入の復旧経路が黙って失敗すると、以降の全ての測定が嘘になる。MSYS で /tmp へ逃がした原本が書けておらず cp が黙って失敗し、変異が累積して数が全て無効になった。復旧の後に diff で原状復帰を確かめよ。check",
        "degree": 0
      },
      {
        "id": "classifier-repair-steals-from-another-road",
        "type": "lesson",
        "label": "分類器の修理は必ず別の行き先から奪う。ある道への誤着を塞ぐ修理は、その判定に使う語彙を広げることで他の道の判定を狭める。実測: 欠陥A を直すため CREATE_RE に建造動詞(設ける|足す|加える|追加|新設|導入|搭載|持たせ|拡張|付ける)を足した結果、isCounsel の打ち消しが効かなくなり『CI に段を設けるべきか検討せよ』型の熟議の願いが counsel を失った — main 0/150 に対し HEAD 120/150。走行は7度の回帰を記録しながら探針を全て reform 軸で作り、counsel 軸を一度も撃たなかった。修理が触れた定数を読む全ての判定経路を列挙し、各経路に探針を持て",
        "degree": 0
      },
      {
        "id": "gate-needs-confusion-matrix-not-a-column",
        "type": "lesson",
        "label": "判定器の門は対角線でなく混同行列で建てよ。『A へ着くべきものが A へ着いた』を n 件撃っても『B へ着くべきものが A へ攫われた』については一言も述べていない。実測: AC-31〜46 の全コーパスが reform 列だけを持ち、counsel 行が 0 件だったため、counsel を 120/150 失う回帰が全門緑のまま通過した(paradise 471/471・counsel 210/210・census/codex/wiring 全 EXIT=0)。道が n 本ある分類器の門は n×n のセルを持ち、対角線の合計と非対角の各セルが 0 であることを両方 assert せよ。新しい語彙を足せば全セルが再計算され、片側だけ直す修理が構造的に不可能になる",
        "degree": 0
      },
      {
        "id": "conduct-lesson-presented-is-not-enforced",
        "type": "lesson",
        "label": "規範の教訓は提示しただけでは守られない。第28条は kind:conduct を『コードで検証できないので提示のみ』としたが、実測でその提示は振る舞いを変えなかった — repair-opens-a-new-surface と mutation-author-is-blind-to-own-net と rework-four-times-is-a-design-debt の三件は critic が ✓ CONDUCT として毎回提示していたのに、同じ走行が同じ形で8度目の回帰を生み四度の差し戻しを払った。ゆえに規範の教訓は『提示』で終わらせず、その教訓が名指す行為を発令書の必須項目へ降ろせ(main を複製して両 head で撃て / 触れた定数を読む全経路を列挙せよ / AC が要求していない軸を最低一つ持て)。測れぬものは点数を付けられないが、命令には変換できる",
        "degree": 0
      },
      {
        "id": "removal-is-a-legitimate-outcome",
        "type": "lesson",
        "label": "撤去は敗北ではなく、四度の実測が導いた正しい結論でありうる。ただし撤去は『元に戻す』ではない — 撤去しても、修理の道中で広げた他の定数は元に戻らない。実測: ENGINE_NAMES を丸ごと撃ち捨てて世間の誤着は 0/20 に戻ったが、同じ走行が欠陥A のために広げた CREATE_RE は撤去の対象外として残り、counsel を 120/150 失う回帰が生き残った。撤去の相は『何を消したか』だけでなく『この走行が触れた全ての定数のうち、消さずに残るものはどれか』を列挙し、残る各々を両 head で撃ち直せ。撤去の安心感が残存差分の検めを省かせる",
        "degree": 0
      }
    ],
    "edges": [
  ...
  "lessons": [
    ...
    },
    {
      "id": "fix-all-siblings-of-the-same-flaw",
      "label": "同じ関数の中で同じ病を片側だけ直すな",
      "check": "同じ関数の中で同じ病を片側だけ直すな — B-1 で files/churn の fail-open を塞いだが同じ関数の bytes は catch {} のまま残り、reflect が同一構造の BLOCK として再発見した。修復のとき『同じ設計上の問いを持つ箇所を全て洗ったか』を必ず問え|applies:paradise-internal|kind:conduct"
    },
    {
      "id": "gate-neutralized-by-or-true",
      "label": "|| true を付けた門は門ではない",
      "check": "CI の段に || true を付けた瞬間、その門は何を食わせても緑になる。楽園は自分でこの戒めを workflow に書きながら 81 行下で破っていた。門の exit code を捨てるなら理由をコメントで書け。理由を書けないなら外せ|applies:paradise-internal|kind:conduct"
    },
    {
      "id": "epoch-marker-needs-a-timestamp",
      "label": "印の有無だけを見る門は印を消せば消える",
      "check": "移行のための恩赦(epoch なしは unobservable)は、印を消しただけの走行にも同じ恩赦を与えた。導入時刻を engine の中に持ち created と比べれば legacy と門の回避を機械で分けられる。恩赦は移行のためであって回避のためではない|applies:paradise-internal|kind:mechanism"
    },
    {
      "id": "abandoned-run-must-ring",
      "label": "見捨てられた走行は機構が名指しする — マージは環の完了ではない",
      "check": "走行帳が domains ratified < 全数 のまま一定期間無音なら engine が名指しせよ (conclave.js audit / runAbandonment)。実測: reform-claude-md-diet は 4/6 のまま 6.9日 放置され実装だけ PR #23 でマージされ 95 コミット何も鳴らなかった。相を見る phaseSilence は running の dispatchedAt しか読まないので、pending のまま忘れられた走行には構造的に映らない — 走行そのものを見る別の目が要る。閾値は STALE/SILENT と別物で 1 箇所にだけ置き、実測(閉じた走行 5 本の全事象間隔 173 件 max=110.4分)を越える境に据える。時刻を読めない走行帳は 0 で埋めず null のまま名指しせよ (第16条)。また reform の走行帳が創造物の倉に迷い込む流出は strayCreations の逆向きであり workspace.strayRuns が見る — 混入は履歴を汚すだけだが流出は審査そのものを迂回する|check|applies:paradise-internal"
    },
    {
      "id": "critic-exemption-self-declaration",
      "label": "門は免除の自己申告を信じない — 空の .paradise-source 1個で創造物の掟3法が素通りした。免除の資格は住所(workspace.js)が裏付ける。免除も却下も必ず名指しで出力に残す (第54条)",
      "check": "門は免除の自己申告を信じない — 空の .paradise-source 1個で創造物の掟3法が素通りした。免除の資格は住所(workspace.js)が裏付ける。免除も却下も必ず名指しで出力に残す (第54条)|applies:paradise"
    },
    {
      "id": "ring-not-closed-by-merge",
      "label": "マージは環の完了ではない — 実装が main へ入っても走行帳が 5/11 のまま週単位で放置された実例がある。域を健全と断ずる前に node graph/conclave.js audit で実測せよ|check",
      "check": "マージは環の完了ではない — 実装が main へ入っても走行帳が 5/11 のまま週単位で放置された実例がある。域を健全と断ずる前に node graph/conclave.js audit で実測せよ|check|applies:paradise"
    },
    {
      "id": "mutation-author-is-blind-to-own-net",
      "label": "自分で打った変異が全部鳴っても網は証明されない",
      "check": "自分で打った変異が全部鳴っても網は証明されない — 変異を設計した者と門を設計した者が同じなら、変異は門の想像の外を撃たない。この環では prove1 が24門で『無音ゼロ』と報告した直後に review が別の8件の無音を見つけ、prove2 が『無音ゼロ』と報告した直後に review-2 が別の39変異で14件の無音を見つけた。二度連続で同じ形で裏切られている。ゆえに prove の主張は『無音ゼロ』ではなく『私が打ったN種の範囲で無音ゼロ』と述べ、網の証明は必ず別の者が別の軸(層・経路・呼び手)で撃つまで保留せよ|applies:paradise-internal|kind:conduct"
    },
    {
      "id": "repair-opens-a-new-surface",
      "label": "修理は次の攻撃面を開く — 差分が塞いだ穴でなく差分が開いた面を撃て",
      "check": "修理は次の攻撃面を開く — 差分が塞いだ穴でなく、差分が新しく可能にしたことを撃て。この環では二度連鎖した: S-2(無制限再帰)の修理が P-1(読めなかった行を0件と偽る)を生み、冪等化そのものが F-1(先回り毒 — main には存在しない穴)を生んだ。どちらも修理の直後の相では見つからず、次の差し戻しで初めて出た。build の後の security/review は『この差分は何を新しく可能にしたか』を専用の節として先に問い、main と HEAD を切り分けて『本改革が作った穴か』を毎回名指せ|applies:paradise-internal|kind:conduct"
    },
    {
      "id": "idempotent-key-can-be-preempted",
      "label": "決定的な冪等鍵は先取りできる — 冪等化は先回り毒の面を必ず開く",
      "check": "決定的な冪等鍵は先取りできる。冪等性の成立条件として鍵の材料から ts を外すと、鍵は誰でも事前に計算でき『同じ観測は二度刻まない』が『先に名乗った者が勝つ』に化ける。攻撃者は要らない — 狂った時計と古い台帳のマージが同じ形で現れる。設計段階で『鍵を先取りされたら何が起きるか』を問い、書き手は skip する前に既存行を検め(走行開始より前に住むか/未来に住むか/秤が書かない鍵を持つか)、正当な観測は必ず追記した上で先回りとして名乗れ。監査は誰も書かない場合に備えて独立に suspect を数えること|applies:gauge"
    },
    {
      "id": "rework-four-times-is-a-design-debt",
      "label": "四度の差し戻しは実装の弱さでなく設計の空白である",
      "check": "四度の差し戻しは実装の弱さでなく設計の空白である。この環の差し戻し4件のうち3件(無音の門・修理が生んだ病・先回り毒)は、design.md が『網をどう設計するか』と『この設計が新しく開く面』を一節も持たなかったことに帰する。設計相は FR ごとに(1)この設計が新しく可能にすること(2)網を誰がどの軸で撃つか(3)自作の変異では届かない層 を明示せよ。設計に一時間払わずに差し戻しで十時間払った|applies:paradise-internal|kind:conduct"
    },
    {
      "id": "selection-must-run",
      "label": "選んだ門を走らせない走行は測定ではない。matched=green+red の恒等式を、選別を行う関数の外側で検めよ|check",
      "check": "選んだ門を走らせない走行は測定ではない。matched=green+red の恒等式を、選別を行う関数の外側で検めよ|check|applies:paradise-internal"
    },
    {
      "id": "warden-outside-selection",
      "label": "門番が絞り込みの対象なら門番ではない。機構を見張る門は、その機構が選び落とせない位置に置け|check",
      "check": "門番が絞り込みの対象なら門番ではない。機構を見張る門は、その機構が選び落とせない位置に置け|check|applies:paradise-internal"
    },
    {
      "id": "artifact-name-is-machine-contract",
      "label": "相の名(discover)と成果物の名(findings.md)は別物。critic は名前で探すので、成果物の名は機械に合わせよ|findings",
      "check": "相の名(discover)と成果物の名(findings.md)は別物。critic は名前で探すので、成果物の名は機械に合わせよ|findings|applies:paradise-internal"
    },
    {
      "id": "child-stdout-exit-truncation",
      "label": "子プロセスが console.log の直後に process.exit() を呼ぶと、POSIX ではパイプ緩衝(64KiB)を越えた stdout が掃き出される前に死に、親は status 0 のまま切れた JSON を読む。Windows は同期書き込みなので再現せず「手元は緑、CI だけ赤」になる。終了コードは process.exitCode で名乗り exit を呼ぶな。また限度超過を「JSON が壊れている」と名乗ってはならない(第34条)",
      "check": "子プロセスが console.log の直後に process.exit() を呼ぶと、POSIX ではパイプ緩衝(64KiB)を越えた stdout が掃き出される前に死に、親は status 0 のまま切れた JSON を読む。Windows は同期書き込みなので再現せず「手元は緑、CI だけ赤」になる。終了コードは process.exitCode で名乗り exit を呼ぶな。また限度超過を「JSON が壊れている」と名乗ってはならない(第34条)|applies:child-process,ci-only-red,archify"
    },
    {
      "id": "gate-red-needs-a-mouth-not-a-wider-gate",
      "label": "門が正しく赤いのに現実は健全なとき、真因は「機構に語彙が無い」ことでありうる。境を伸ばす/例外を足す前に、engine に足りない口(記録の語)を探せ。口には濫用を禁じる錬(実在・非空・二度指し禁止・状態不変・註釈の実質)を掛け、逆の門(口が無い場合は今まで通り赤い)で固定する|check",
      "check": "門が正しく赤いのに現実は健全なとき、真因は「機構に語彙が無い」ことでありうる。境を伸ばす/例外を足す前に、engine に足りない口(記録の語)を探せ。口には濫用を禁じる錬(実在・非空・二度指し禁止・状態不変・註釈の実質)を掛け、逆の門(口が無い場合は今まで通り赤い)で固定する|check|applies:paradise-internal"
    },
    {
      "id": "both-branches-of-a-rule",
      "label": "判定規則が二枝を持つなら、片枝の証明は他枝について何も語らない。弱い名に二条件を課して誤着を塞いだ走行が、強い名 26 語を無条件で通す口をそのまま残し、世間の願い 21/21 を reform へ攫った(main では 0/21)。片側を直したら必ず反対側を同じ探針で撃て",
      "check": "判定規則が二枝を持つなら、片枝の証明は他枝について何も語らない。弱い名に二条件を課して誤着を塞いだ走行が、強い名 26 語を無条件で通す口をそのまま残し、世間の願い 21/21 を reform へ攫った(main では 0/21)。片側を直したら必ず反対側を同じ探針で撃て|applies:forge"
    },
    {
      "id": "regression-needs-both-heads",
      "label": "回帰か否かは HEAD だけでは判じられない。同じ探針を main と HEAD の両方で撃って初めて本走行の産物と断定できる。第38条の前後比較を成果物の数にしか使わず、振る舞いに使わなかった走行は、自分が作った誤着を仕様と見誤る",
      "check": "回帰か否かは HEAD だけでは判じられない。同じ探針を main と HEAD の両方で撃って初めて本走行の産物と断定できる。第38条の前後比較を成果物の数にしか使わず、振る舞いに使わなかった走行は、自分が作った誤着を仕様と見誤る|applies:reform"
    },
    {
      "id": "corpus-blind-to-its-own-half",
      "label": "門のコーパスが規則の片枝しか含まないなら、門は永久に緑のまま面の半分を素通しする。AC-31 の 24 件は全て弱い名で作られ、強い名は 0 件だった。コーパスは規則の枝ごとに数えて示せ。故障注入の変異も同じ — 全変異が片枝を狙うなら、他枝について何も証明していない",
      "check": "門のコーパスが規則の片枝しか含まないなら、門は永久に緑のまま面の半分を素通しする。AC-31 の 24 件は全て弱い名で作られ、強い名は 0 件だった。コーパスは規則の枝ごとに数えて示せ。故障注入の変異も同じ — 全変異が片枝を狙うなら、他枝について何も証明していない|applies:gate"
    },
    {
      "id": "filter-in-a-gate-is-a-silent-exemption",
      "label": "網羅を主張する門の中の filter は沈黙の免除である。graph/*.js の網羅を撃つ門が n.length >= 4 で kg を黙って落とし、kg が語彙表に無いまま門は永久に緑だった。門が測る対象と門が主張する範囲がずれていないか、除外した件数を必ず出力に名乗らせよ",
      "check": "網羅を主張する門の中の filter は沈黙の免除である。graph/*.js の網羅を撃つ門が n.length >= 4 で kg を黙って落とし、kg が語彙表に無いまま門は永久に緑だった。門が測る対象と門が主張する範囲がずれていないか、除外した件数を必ず出力に名乗らせよ|applies:gate"
    },
    {
      "id": "strength-of-evidence-sets-the-gate",
      "label": "判定の枝が複数の証拠を持つとき、証拠の強さが許す条件の広さを決める。強い証拠(engine の強い固有名)に弱い証拠と同じ条件を機械的に写すと片方が壊れる — 強い名に BUILD_RE だけを課すと『conclave の毒を除く』が死ぬ。ゆえに強い名には BUILD_RE ∪ MEND_RE を課した。check",
      "check": "判定の枝が複数の証拠を持つとき、証拠の強さが許す条件の広さを決める。強い証拠(engine の強い固有名)に弱い証拠と同じ条件を機械的に写すと片方が壊れる — 強い名に BUILD_RE だけを課すと『conclave の毒を除く』が死ぬ。ゆえに強い名には BUILD_RE ∪ MEND_RE を課した。check|applies:forge"
    },
    {
      "id": "mutate-the-gate-not-only-the-impl",
      "label": "故障注入が実装側だけを狙うなら、門のコーパスの穴は永久に見つからない。門(コーパス・表との照合)の側を壊す変異を必ず一つ以上持て。F-4 は変異 3 件が全て実装側だったために、29 件のコーパスが守る面の半分を撃っていないことを誰も見なかった。check",
      "check": "故障注入が実装側だけを狙うなら、門のコーパスの穴は永久に見つからない。門(コーパス・表との照合)の側を壊す変異を必ず一つ以上持て。F-4 は変異 3 件が全て実装側だったために、29 件のコーパスが守る面の半分を撃っていないことを誰も見なかった。check|applies:gate"
    },
    {
      "id": "corpus-machine-checked-against-table",
      "label": "『表の全語を撃っている』という主張は、表とコーパスを機械で照合して初めて真になる。人が足し忘れれば F-4 は再演する。ENGINE_NAMES_STRONG の全語 ⇔ コーパスの過不足を両方向で assert せよ。check",
      "check": "『表の全語を撃っている』という主張は、表とコーパスを機械で照合して初めて真になる。人が足し忘れれば F-4 は再演する。ENGINE_NAMES_STRONG の全語 ⇔ コーパスの過不足を両方向で assert せよ。check|applies:gate"
    },
    {
      "id": "restore-path-must-be-verified",
      "label": "故障注入の復旧経路が黙って失敗すると、以降の全ての測定が嘘になる。MSYS で /tmp へ逃がした原本が書けておらず cp が黙って失敗し、変異が累積して数が全て無効になった。復旧の後に diff で原状復帰を確かめよ。check",
      "check": "故障注入の復旧経路が黙って失敗すると、以降の全ての測定が嘘になる。MSYS で /tmp へ逃がした原本が書けておらず cp が黙って失敗し、変異が累積して数が全て無効になった。復旧の後に diff で原状復帰を確かめよ。check|applies:gate"
    },
    {
      "id": "classifier-repair-steals-from-another-road",
      "label": "分類器の修理は必ず別の行き先から奪う。ある道への誤着を塞ぐ修理は、その判定に使う語彙を広げることで他の道の判定を狭める。実測: 欠陥A を直すため CREATE_RE に建造動詞(設ける|足す|加える|追加|新設|導入|搭載|持たせ|拡張|付ける)を足した結果、isCounsel の打ち消しが効かなくなり『CI に段を設けるべきか検討せよ』型の熟議の願いが counsel を失った — main 0/150 に対し HEAD 120/150。走行は7度の回帰を記録しながら探針を全て reform 軸で作り、counsel 軸を一度も撃たなかった。修理が触れた定数を読む全ての判定経路を列挙し、各経路に探針を持て",
      "check": "分類器の修理は必ず別の行き先から奪う。ある道への誤着を塞ぐ修理は、その判定に使う語彙を広げることで他の道の判定を狭める。実測: 欠陥A を直すため CREATE_RE に建造動詞(設ける|足す|加える|追加|新設|導入|搭載|持たせ|拡張|付ける)を足した結果、isCounsel の打ち消しが効かなくなり『CI に段を設けるべきか検討せよ』型の熟議の願いが counsel を失った — main 0/150 に対し HEAD 120/150。走行は7度の回帰を記録しながら探針を全て reform 軸で作り、counsel 軸を一度も撃たなかった。修理が触れた定数を読む全ての判定経路を列挙し、各経路に探針を持て|applies:forge"
    },
    {
      "id": "gate-needs-confusion-matrix-not-a-column",
      "label": "判定器の門は対角線でなく混同行列で建てよ。『A へ着くべきものが A へ着いた』を n 件撃っても『B へ着くべきものが A へ攫われた』については一言も述べていない。実測: AC-31〜46 の全コーパスが reform 列だけを持ち、counsel 行が 0 件だったため、counsel を 120/150 失う回帰が全門緑のまま通過した(paradise 471/471・counsel 210/210・census/codex/wiring 全 EXIT=0)。道が n 本ある分類器の門は n×n のセルを持ち、対角線の合計と非対角の各セルが 0 であることを両方 assert せよ。新しい語彙を足せば全セルが再計算され、片側だけ直す修理が構造的に不可能になる",
      "check": "判定器の門は対角線でなく混同行列で建てよ。『A へ着くべきものが A へ着いた』を n 件撃っても『B へ着くべきものが A へ攫われた』については一言も述べていない。実測: AC-31〜46 の全コーパスが reform 列だけを持ち、counsel 行が 0 件だったため、counsel を 120/150 失う回帰が全門緑のまま通過した(paradise 471/471・counsel 210/210・census/codex/wiring 全 EXIT=0)。道が n 本ある分類器の門は n×n のセルを持ち、対角線の合計と非対角の各セルが 0 であることを両方 assert せよ。新しい語彙を足せば全セルが再計算され、片側だけ直す修理が構造的に不可能になる|applies:gate"
    },
    {
      "id": "conduct-lesson-presented-is-not-enforced",
      "label": "規範の教訓は提示しただけでは守られない。第28条は kind:conduct を『コードで検証できないので提示のみ』としたが、実測でその提示は振る舞いを変えなかった — repair-opens-a-new-surface と mutation-author-is-blind-to-own-net と rework-four-times-is-a-design-debt の三件は critic が ✓ CONDUCT として毎回提示していたのに、同じ走行が同じ形で8度目の回帰を生み四度の差し戻しを払った。ゆえに規範の教訓は『提示』で終わらせず、その教訓が名指す行為を発令書の必須項目へ降ろせ(main を複製して両 head で撃て / 触れた定数を読む全経路を列挙せよ / AC が要求していない軸を最低一つ持て)。測れぬものは点数を付けられないが、命令には変換できる",
      "check": "規範の教訓は提示しただけでは守られない。第28条は kind:conduct を『コードで検証できないので提示のみ』としたが、実測でその提示は振る舞いを変えなかった — repair-opens-a-new-surface と mutation-author-is-blind-to-own-net と rework-four-times-is-a-design-debt の三件は critic が ✓ CONDUCT として毎回提示していたのに、同じ走行が同じ形で8度目の回帰を生み四度の差し戻しを払った。ゆえに規範の教訓は『提示』で終わらせず、その教訓が名指す行為を発令書の必須項目へ降ろせ(main を複製して両 head で撃て / 触れた定数を読む全経路を列挙せよ / AC が要求していない軸を最低一つ持て)。測れぬものは点数を付けられないが、命令には変換できる|applies:paradise-internal|kind:conduct"
    },
    {
      "id": "removal-is-a-legitimate-outcome",
      "label": "撤去は敗北ではなく、四度の実測が導いた正しい結論でありうる。ただし撤去は『元に戻す』ではない — 撤去しても、修理の道中で広げた他の定数は元に戻らない。実測: ENGINE_NAMES を丸ごと撃ち捨てて世間の誤着は 0/20 に戻ったが、同じ走行が欠陥A のために広げた CREATE_RE は撤去の対象外として残り、counsel を 120/150 失う回帰が生き残った。撤去の相は『何を消したか』だけでなく『この走行が触れた全ての定数のうち、消さずに残るものはどれか』を列挙し、残る各々を両 head で撃ち直せ。撤去の安心感が残存差分の検めを省かせる",
      "check": "撤去は敗北ではなく、四度の実測が導いた正しい結論でありうる。ただし撤去は『元に戻す』ではない — 撤去しても、修理の道中で広げた他の定数は元に戻らない。実測: ENGINE_NAMES を丸ごと撃ち捨てて世間の誤着は 0/20 に戻ったが、同じ走行が欠陥A のために広げた CREATE_RE は撤去の対象外として残り、counsel を 120/150 失う回帰が生き残った。撤去の相は『何を消したか』だけでなく『この走行が触れた全ての定数のうち、消さずに残るものはどれか』を列挙し、残る各々を両 head で撃ち直せ。撤去の安心感が残存差分の検めを省かせる|applies:reform"
    }
  ],
  "creations": [
    {
