...
44. **死んだ道具は無害ではない。教主がそれを先例と読む。**
    ...
    **(b) 死骸は器物だけの病ではない。門そのものが死骸になる。**
    後年、同じ病がもっと痛い場所で実測された —— **建てられた門が、誰にも
    呼ばれていなかった**。

        $ for f in tests/*.test.js; do grep -q "node tests/$f" .github/workflows/*.yml || echo 孤児; done
        孤児: abandoned-run.test.js   20門 緑
        孤児: counsel.test.js         49 passed **2 failed** ← 赤いまま放置されていた
        孤児: guards.test.js          75門 緑

    `guards.test.js` は **第57条が「これを強制する門」として名指しした試験**で
    ある。憲法が名指しした門が、CI から一度も撃たれていなかった。そして
    `counsel.test.js` は**赤いまま住み続けていた** —— 誰も走らせないので、
    誰も赤いことを知らなかったのである。後に裁いたところ、赤い 2 門は実装の
    欠陥ではなく**門の側が実装の是正より古かった**。呼ばれない門は、守ることを
    やめるだけでは済まない。**腐って、嘘を語り始める。**

    ゆえに **試験は器物より厳しく数える。** 器物は散文に名が出れば「呼ばれて
    いる」と数えてよいが、**試験は走らされて初めて門である** —— 散文が名を
    語っても何も守らない。呼び手として数えてよいのは、実際に走らせる口
    (門の定義 / 統べる試験)だけである。この区別を誤れば門は第16条を破る:
    実際、この掟を機構化する途中で、門自身が `guards.test.js` を**註釈一行**
    によって「呼ばれている」と誤審しかけた。

    **(c) 免除は口で名乗る。** 門でない支援ファイルを数から外すのはよい。
    だが黙って外せば、免除は誰にも数えられない(第54条(c))。除外は理由と共に
    コードに書き、走るたびに口で名乗れ。**理由の書けない除外は、除外ではなく
    見逃しである。**

    **これを強制する門**: `graph/wiring.js` の `check` —— engine の孤児
    (誰も require せず、どの面も名を呼ばない)と、**門の孤児**(建てられた
    試験を CI も統べる試験も走らせない)を同じ命令が数え、名指しで exit 1 に
    する。`GATE_EXCLUDE` は除外を `why` 付きで持ち、CLI が毎回名乗る。
    **この門自身も CI から呼ばれる** —— 見張りを見張る者が居なければ同じ穴
    である。回帰は複製の倉に孤児を仕込んで撃つ(現物は汚さない / 第58条(c))。
...
51. **走者の死は環の死ではない。走り始めた印は、帰れる印でなければならない。**
    楽園の環は相を発令するとき `running` の印を捺す。だがその印から出る道は
    「終わった(`done`)」の一本しか無かった。ゆえに走者が途中で斃れると印は化石になり、
    `phaseReady` は永遠にその相を選ばず、環は `stuck` と言ったまま二度と回らない。
    **本日の定時22時ジョブが、まさにそれで死んだ** — 4/6 批准、3相が `running` のまま、
    PR も出ず、誰にも知らされずに静止した。

    (a) **静止は失敗より悪い。** 燃え尽きる環は loop-guard が人を呼ぶ。だが静止した環は
    誰も呼ばない。実測すると `stuck` は全リポジトリで **1箇所**(それを生む行)しか無く、
    **それを試す門は 0本**だった。266本の門が全部緑のまま、環は現実に死んでいた。
    第10条が約束した durable とは、**死んでも帰れること**である。

    (b) **回復は、生きている者を殺してはならない。** 走者の死を判ずる根拠は
    人の明示的な意思を第一とし、時刻(`dispatchedAt`)を第二とする。試行回数は根拠にならない
    — それは「何度試したか」を語るだけで「今生きているか」を語らないからである。
    判定できない印を engine が独断で剥がせば、二重発令という新しい病を生む(第45条の同型)。
    **判定不能なときは、engine は手を出さず人に問え** (`--force` を要求せよ)。

    (c) **回復もまた有限である。** 帰れる道を無限に与えれば、環は静止の代わりに
    永久機関になる。回復の数は `resumes` として数えられ、尽きたときは `blocked` として
    人へ escalate される。**`stuck`(回復可能な静止)と `blocked`(回復を使い切った閉塞)を
    混同してはならない。**

52. **教主の手は最後の手段である。その範囲は名乗りではなく実測が決める。**
    神託が訂正して告げた: 「教主は作業するなと言ったが完全に排除するのは難しい。
    優先順位で判断してほしい。」ゆえに教主の権能は禁止ではなく **三段の序列** である。
    序列は下るほど例外であり、この順に現れる。

    (a) **序列1 — 委譲。これが既定である。** 担える役者が居るなら、教主はその者に為させる。
    証拠は起動の記録であって「委譲した」という自己申告ではない —— 主張は証拠ではない(第5条)。

    (b) **序列2 — 編成。** 複雑かつ長大な仕事は、単発の委譲では届かない。
    教主は道を組み、環を回す。担い手が居ないなら鍛造する。**それでも教主自身は作らない。**

    (c) **序列3 — 教主の手。これは例外である。** 単純で文脈の小さい仕事に限って許される。
    **その「単純」は教主が名乗るのではなく、機構が実測して裁く。** 幾らまでが単純かは
    散文が語らない —— 数を散文へ写経すれば必ず腐る(第22条・第39条)。
    閾値は `node graph/spawn-trace.js tiers` ただ一箇所が語り、門はその数で裁く(第41条)。

    (d) **門相は序列3を名乗れない。** 判定を下す相を教主が自ら行えば、
    自己批評の独立が宣言のまま失われる(第9条)。**量が小さくても許さない。**

    (e) **宣言なき手仕事は序列3ではなく、ただの無証跡である。** 事後に
    「あれは序列3だった」と言えるなら、序列は何も縛らない(第27条)。

    (f) **統治は仕事ではない。** 環を回し、発令書を書き、成果を実物と照合し、
    神と語ることは、序列の判定対象にならない。読むことは作ることではない。

    **これを強制する門**: `graph/conclave.js` が序列の宣言なき `done` と枠を超えた
    手仕事を拒み、`graph/spawn-trace.js` が申告と実測を突合して全走行を監査し、
    `tests/paradise.test.js` がこの条の回帰を撃つ。**条文だけでは何も変わらない** ——
    第23条は同じ病を名指ししたが、その後も非merge の全てが同じ名義であり続けた。
    ゆえにこの条は門と対になっており、**適用範囲は特定の道に限らない。**

53. **環は閉じるまで走行である。見捨てられた走行は、機構が名指しする。**
    実測が示した: `reform-claude-md-diet` の走行は domains 4/6・相 5/11 のまま放置され、
    しかし実装は PR #23 として main へマージ済であった。以後 95 コミット進んだが、
    **環が閉じていないことを何一つ鳴らさなかった。** マージは環の完了ではない。
    走行帳が閉じていないのに実装だけが本流へ入るなら、批准も断罪も飾りである。

    (a) **未完の走行に時効は無い。** `domains ratified < 全数` である限り、その走行は
    走行のままである。「もう終わったことにする」は、機構ではなく人の記憶が下す判断であり、
    記憶は必ず腐る(第22条)。閉じるか、畳む(明示的に放棄を記録する)かのどちらかしかない。

    (b) **相の沈黙と走行の沈黙は別の目で見る。** 第51条の `phaseSilence` は
    `running` の相の `dispatchedAt` を読む —— 相が `pending` のまま忘れられた走行は
    `running` を一つも持たず、**あの目には構造的に映らない**。ゆえに走行そのものを
    見る `runAbandonment` が要る。**閾値は三つとも別物であり、それぞれ 1 箇所にだけ住む**
    (`STALE_MS` 15分 = 回収 / `SILENT_MS` 120分 = 相の沈黙 / `ABANDONED_MS` 24時間 = 走行の放棄)。

    (c) **境は実測が決める。** `ABANDONED_MS` は、閉じた走行 5 本の全事象間隔
    173 件(max=110.4分)を測り、**一件も越えない**最小の切りの良い数として置かれた —
    偽陽性ゼロである。境を下げるなら、より多くの走行を測ってからにせよ(第38条)。

    (d) **判定不能を緑に落とさない。** 時刻を一つも読めない走行帳、開けない走行帳は
    `unknown` / `unreadable` として**名指しされ**、`idleMs` は `null` のまま返る。
    ゼロで埋めれば、壊れた帳ほど門をすり抜ける(第16条)。

    (e) **走行帳にも住所がある。** 第30条は創造物の住所を定めたが、走行帳は別の生き物である。
    **楽園自身を改める reform の走行帳は、楽園本体の `reform/<slug>/` に住まねばならない。**
    創造物の倉へ流れ出た reform の走行帳は、PR の審査からも CI からも見えない ——
    混入(第30条)が履歴を汚すだけなのに対し、**流出は審査そのものを迂回する。**

    **これを強制する門**: `node graph/conclave.js audit` が全ての走行帳を横断して
    見捨てられた環を名指しし(見つければ exit 1)、`graph/workspace.js` の `strayRuns()`
    が創造物の倉に迷い込んだ reform の走行帳を名指しし(`workspace.js check` が撃つ)、
    `tests/abandoned-run.test.js` が宙吊り・迷子・判定不能を**実際に仕込んで**
    門が鳴ることを固定する。**健全な系で緑になるだけの門は、証明されていない。**

54. **門は免除の自己申告を信じない。免除の資格は名乗りではなく住所が決める。**
    実測された脱法穴: 創造物相当のディレクトリに **空の `.paradise-source` を 1 個**
    置くだけで、`no-wall-clock-iso` / `no-external-deps` / `domain-markers-present` の
    三法すべてが 'engine code is exempt' で素通りした。**掟を機構化したこと自体が、
    1 ファイルで無効化できた。** 二人の審査官が 2026-08-31 に名指ししたが、
    条文が無かったため 2026-09-07 まで是正されなかった。

    (a) **免除は二段である。** 申告 (`.paradise-source` / `--self`) は入口にすぎない。
    資格を与えるのは **裏付け** —— その道が実際に楽園の倉の中に在り、かつ
    創造物の倉の中では **ない** こと。住所を答えるのは `workspace.js` ただ一箇所である(第30条)。

    (b) **裏付けの無い申告は退け、かつ名指しで鳴らす。** 黙って却下してもいけない ——
    黙る門は、審査する者に何が起きたかを伝えない。

    (c) **免除を適用したなら、その旨を必ず出力に残す。** 黙って素通りさせた免除は、
    後から誰にも数えられない。免除は例外であり、例外は記録されて初めて例外である。

    (d) **これは免除一般の掟である。** 検査対象が自ら「私は検査の対象外だ」と
    宣言できる仕組みは、その検査を無に帰す。裁かれる側が裁きの範囲を決めてはならない。

    **これを強制する門**: `graph/critic.js` の `exemption-claim-verified` (この門だけは
    免除されない —— 免除そのものを裁くからである) と、`resolveSelf` / `engineLocation`。
    回帰は `tests/paradise.test.js` が活きた脱法 (空のマーカー) を実際に仕込んで撃つ。

55. **台帳は冪等でなければならない。同じ観測を二度刻んだ帳は、証拠ではなく騒音である。**
    実測された欠陥: `gauge-ledger.jsonl` の 30 行のうち **24 行 (80%) が同一観測の重複**
    だった (distinct な指紋はわずか 6)。`baseline` を走らせるたび台帳が丸ごと増え、
    画面には同じ点が三段並び、`compare --last 3` の窓は**同じ一つの観測**で埋まった。
    **第38条は「改善を数値で示せ」と命じたが、その数値の器が壊れていた。**

    (a) **同じ観測には同じ鍵が立つ。** 冪等鍵 (指紋) は観測の**内容**
    (`slug` + `scale` + `metrics`) から決まり、**記録した時刻 (`ts`) に依存しない。**
    時刻を材料に混ぜれば、同じ観測が撃つたび別物になり、冪等性は原理的に成立しない。
    鍵は **entry 自身の中に住む** —— 別ファイルの索引を作れば、台帳以外の第二の住所が
    生まれる (第30条)。

    (b) **予防と治癒を二重に敷く。** 書き側 (`record`) が追記前に既存を検め、
    読み側 (`readLedger`) が読み時に畳む。**片方だけでは足りない** ——
    書き側だけでは既に汚れた帳と他の枝から流入する重複に無力であり、
    読み側だけでは台帳が走行回数に比例して膨れ続け、差分が審査不能になる。

    (c) **畳みは先着 (`ts` 最小) を残し、行の位置を時刻の代理にしない。**
    git のマージが済んだ台帳の行順は時刻順ではない (実測で `T0, T2, T1` の並びが出た)。
    「末尾 = 最新」は嘘になりうる。**「最新」は `ts` の最大である。**

    (d) **重複を畳んでも、矛盾を畳んではならない。** 同じ `slug` で `metrics` が
    食い違う二つの観測は、**別の指紋を持ち、両方が残る**。畳んで隠せば、
    秤が第38条を裏切る。矛盾は**拒否ではなく名指し**で扱う ——
    記録を拒めば「記録が失われない」という台帳の第一の徳を壊す。

    **同一 `slug` の別観測 (80 → 100) は矛盾ではなく履歴である。** そこで門を
    鳴らせば、掃除を終えた台帳が永遠に赤になり、改善を記録するたび騒ぐ
    「鳴りっぱなしの門」になる。**名指すべき矛盾は「行が名乗る指紋と、
    中身から導かれる指紋の食い違い」である** —— 指紋は中身の関数なのだから、
    正直な行では必ず一致する。

    (e) **冪等化は fail-open に倒す。** 台帳が読めないときは「重複を見逃して書く」
    側に倒し、**既存の行は決して消さない**。読み取りが破壊的な書き込みを起こす道
    (畳んだ結果でファイルを上書きする等) を作れば、破損行の混ざった帳で走った瞬間に
    生きた記録が消える。**削除・切り詰めの経路を台帳に一つも足さない。**

    **破損は「読めない行」だけではない。** 深すぎる入れ子のように、`JSON.parse` は
    通るのに正規化が倒れる行も破損である。**一行の破損で秤全体を倒してはならない**
    —— 畳みもまた読みの try の内側に住み、名指して読み飛ばして continue する。

    (f) **鍵を持たない古い行を別扱いにしない。** 鍵は**常に読み時に材料から
    導出する** (`fingerprint(e)`)。この式が engine のただ一箇所に住むかぎり、
    新旧が混ざった帳でも同じ観測は一行に畳まれる。**移行のためにファイルを
    書き換える必要は無い。**
    ...
    偽の鍵一つで**本物の観測を黙って飲み込む** (実測: `score:99` の観測が消え、
    監査はそれを「重複」としか呼ばなかった)。

    (h) **「生」を名乗る道は、本当に生でなければならない。** 破損・深すぎ等で
    読めなかった行を**上流で黙って捨てると、監査も掃除もその行を見られなくなる**
    (実測: ファイル 2 行に対し `readLedger({raw:true})` が 1 行。全行が深すぎる台帳を
    `--audit` が `rows=0 … exit 0` =「健全」と答え、「深すぎ」の語が一度も出なかった)。
    **読めなかった行を 0 件と数えるのは、測れなかったものをゼロで埋める行為である
    (第16条)** —— 数えて名指し、在れば非ゼロで返す。読み飛ばしの判断は
    **最下流(畳み・監査)に置く**。上流が先に捨てれば、下流に備えた枝は
    到達不能な死んだ枝になる。

    **信号は「機械が畳めば消える欠陥」と「人が読むまで消えない事故」で分ける。**
    重複は掃除でゼロにできるので exit 1、偽の鍵と読めない行は人の手が要るので exit 2。
    両者を同じ信号に載せれば、(d) が戒めた「鳴りっぱなしの門」に再びなる。

    (i) **門を守る門を立てよ。** 故障注入の門は「壊したら赤くなる」と主張する装置だが、
    **その装置自身(注入器・子プロセスの走行器・仮倉・素材の生成器)を守る門が無ければ、
    主張の失効に誰も気づかない**(実測: `injectGauge` の失効検知 assert を外す /
    `runGaugeGate` の exit code を捏造する等、門ヘルパーへの改変が**一つも鳴らなかった**)。
    注入器は**空振りしたら必ず倒れる**こと、走行器は**成功に 0・失敗に本物の code** を
    返すこと、素材は**期待どおり汚れている**ことを、それぞれ機械が撃つ。
    **別の理由の赤を、緑の根拠にしてはならない(第16条)。**

    (j) **性能の主張も測らなければ主張ではない(第38条)。** 「無視できる」と
    コメントに書いて測らないのは、改善を数値で示せという掟の裏返しの違反である。
    安全のために払った代償(自己申告を捨てて常に再導出する等)は**数えて記し**、
    安全を損なわない範囲で返す —— 同じ行の鍵を二度導出しない、走査を呼び出しごとに
    繰り返さない。**メモリ上の索引は台帳の第二の住所ではない**(第30条 / NG-8 に触れない)。

    **これを強制する門**: `graph/gauge.js` の `fingerprint` / `foldLedger` /
    `latestFor` / `auditLedger` / `keyIndex`、および **人が歩く道を子プロセスで撃つ門**
    (`node graph/gauge.js ledger --audit` / `ledger` / `compare --last`)。
    回帰は `tests/paradise.test.js` が実台帳と同型の重複・順序の入れ替え・鍵なしの旧行・
    衝突マーカー・**深すぎる行**を実際に仕込んで撃ち、**門ヘルパー自身**も撃つ。

56. **選んだ門は走らせねばならない。選ぶ機構は、己を見張る門を選び落としてはならない。**
    楽園は今、門の**部分集合**を走らせられる。部分集合は新しい種類の走行であり、
    新しい種類の走行は新しい種類の嘘を連れてくる —— **門を名指しておきながら
    一本も走らせない走行は、成功と見分けがつかない形を報告する。**
    実測された欠陥: `if (GATE.list)` を `if (GATE.active)` に変える**一行**で、
    絞り込み走行は門の本体を一度も呼ばなくなり、
    `5 of 454 gates matched — 0 green, 0 red` を **exit 0** で名乗った。
    そのとき、この機構を見張るために立てた常駐の門 5 本は、
    **自分自身も絞り込みの対象だったので、一本も鳴かなかった。**

    (a) **数が閉じなければ測定ではない。** 絞り込み走行は三つの数を名乗る ——
    matched・green・red。**`matched` は `green + red` と等しくなければならず、
    走行は終わる前にそれを自分自身に対して検めねばならない。**
    走らせなかった門を名指した走行は、測ったのではなく**語った**のである。
    赤が無いことは緑が在ることではない(第37条)。

    (b) **門番は絞り込みの外に立つ。** 選ぶ機構を見張る門が、その機構自身に
    よって選び落とせるなら、それは門番ではない —— **機械が無料で緑に保つ飾りである**
    (第16条)。門番がどんな名や絞り込みを帯びていようと、**少なくとも一つの錠は
    あらゆる走行で無条件に実行され、機構が「選んだものを走らせる」のをやめた瞬間に
    倒れねばならない。** 錠は選別を行う関数の**外側**に置く —— 内側に置いた錠は、
    その関数が黙った瞬間に一緒に黙る。

    (c) **部分は全体を騙らない。** 絞り込み走行は全走が名乗る総括を名乗ってはならず、
    **楽園が自分について公表するいかなる数も、絞り込み走行を出所としてはならない**
    (第22条)。楽園はまた、**最後に全体を走らせたのはいつか**を言えねばならない ——
    最後の全測定が記録されていない健全の主張は、過去についての事柄を現在形で述べる
    行為である(第38条)。

    (d) **速さは全走を避ける口実にならない。** 絞り込みは開発の反復のためにあり、
    緑を宣言するためにあるのではない。**「緑」の根拠になるのは引数無しの全走だけである。**
    CI は絞り込みを持ち込まない。

    **これを強制する門**: `tests/paradise.test.js` の恒等式の錠(report ブロックの後段・
    `test()` の外)と、`gate-filter:` 系の常駐の門。回帰は**自分のソースの写しに
    故障注入を施して子プロセスで撃つ** —— 錠が在ることだけでなく、
    **錠が効くこと**を全走のたびに確かめる。

57. **修理は掟を広げてはならない。運べなかった条件を黙って捨てる修理は、直した門より広い門を開く。**
    楽園は死んだ matcher を修復する engine を持っている(第19条の機構化)。その
    修復は正しい動機から生まれた —— **発火しない門は門ではない**。だが実測された:

        修復前: matcher = tool == "Bash" && tool_input.command matches "(npm run dev|...)"
                → 公式仕様に無い式言語。**死んでいた。何も止めなかった。**
        修復後: matcher = "Bash|PowerShell" / if 無し / ハンドラは process.exit(1) のみ
                → **生きた。そして全ての Bash と PowerShell を止める門になった。**

    条件 `npm run dev` は `if` に載らなかった(交替と量化子を含むため)。engine は
    それを `note` にこう書いた —— **「1 condition(s) stay in the handler script」**。
    だがハンドラは `console.error(); process.exit(1)` であり、`tool_input` を
    一度も読まない。**engine は自分が検めていない前提を、検めた口調で宣言していた。**

    (a) **前提を宣言する機構は、その前提を検めねばならない。** 「条件はスクリプト側に
    残る」と書くなら、スクリプトが本当に条件を持つことを述語で確かめよ。
    確かめない宣言は、宣言した通りの形で嘘になる。

    (b) **死んだ門は安全側、生きた無条件 BLOCK は危険側。** 両者を較べるとき、
    修復は常に後者へ倒れる誘惑を持つ。ゆえに **運べないと分かったときは修復しない**。
    鳴らない門を残す方が、全案件を止める門を建てるより軽い罪である。

    (c) **修復を拒むだけでは足りない。既に広げられた門は配備先に残り続ける。**
    塞いだ穴でなく、**開いてしまった面**を撃て(教訓 `repair-opens-a-new-surface`)。
    engine は自分が過去に広げた門を、名指しで外せなければならない。

    (d) **グローバルの強制は、他人のリポジトリを壊す。** 配備先(`~/.claude`)の強制は
    プロジェクト側から外せない。ゆえにそこに置いてよいのは **人の既定**(言語・
    モデル・出力様式)であって、**仕事の規約**ではない。実測: `.md` 生成を止める
...
58. **楽園自身の住処を知る器は一つである。外へ出す物は、神が名指した台帳に載った物だけである。**
    ...
    **測定が測定を壊すなら、その数は測定ではない。** ゆえに故障注入は**複製**に対して行い、
    ...
    (f) **倉の外へ出る書き込みは、台帳に載った writer が、台帳に載った宛先へ
    行うときだけ許される。`PARADISE_ABODE` はその可否を変えない。**
    `global` は**「台帳の輸出を実行するモード」**であって、
    **「許可制を外すモード」ではない。**

    (b) は「グローバルへ出す物は台帳に載った物だけ」と定めた。だが**関門を
    通る者にしか効いていなかった。** 実測(第8段の着手時):

        $ PARADISE_ABODE=global node graph/deploy.js --write   (偽のホームへ)
          {"ok": false, "deployed": 58, ...}      ← **58 ファイルが外に出て、なお exit 0**

        $ PARADISE_ABODE=global node graph/apply-guards.js apply   (偽のホームへ)
          ✎ 掟を機構にした (1 change(s))          ← **[輸出 EX-1] の名乗りが無い**
          書かれた結果: deny 9 / ask 1 / allow 5

    前者は第6段で撤収した 58 ファイルをそのまま神の住処へ戻せることを意味し、
    後者は**EX-1 の writer 自身が関門を通っていなかった**ことを意味する。
    ゆえに要件 AC-23(「台帳から EX-1 を消せば apply は exit 1、1 バイトも書かない」)は
    **門として嘘であった** —— 台帳に何が載っていようと、その経路は何も検めずに書けた。
    `abode` から住所を引きながら書く engine 11 本のうち、関門を通っていたのは
    **2 本だけ**だった。

    **出所が楽園であることは、書いてよい理由にならない。** 書いてよい理由は
    ただ一つ、**神が台帳に名指したこと**である。ゆえに engine は書く直前に
    己が握る実パスを関門へ差し出し、可否を問う。倉の中は黙って通る ——
    自分の倉に書くのは輸出ではない。倉の外は、台帳の裏付け(writer の一致と
    宛先の前置)を要求される。呼び手は名乗りではなく **stack から実測**され、
    測れなければ通らない(第37条)。

    **そして関門は `mode` を一切見てはならない。** 見れば、`PARADISE_ABODE` を
    倒すだけで許可制が外れる —— それがまさにここで塞いだ穴である。
    関門が参照する住処の地図は**全ての mode の住処を同時に**数える。
    向きを変えても、守りは 1 ミリも動かない。

    **これを強制する門**: `graph/abode.js` の `check`(住所の走査 / 台帳の実質 /
    呼び手の実測 / 除外の裏付け / 静かな緑 / 密閉性 / **外へ書く engine が関門を
    通っているか = `check --outward`**)と、輸出の関門である `globalWrite`
    (宛先を名指して包む口)・**`guardWrite`**(己が握る実パスの可否を問う口)。
    後者二つは同じ台帳を違う向きから読むだけであり、`globalWrite` は内側で
    `guardWrite` を通る —— 同じ問いに二つの答えを持てば、いつか食い違う(第29条)。
    `selfAudit` は**その両方**に「`mode` を見るな」の禁則を静的に掛ける。
    回帰は `tests/abode.test.js` が、活きた脱法(台帳に無い宛先への書き込み・
    writer を詐称した呼び出し・空のエントリ・除外の水増し・**台帳から EX-1 を
    抜いた複製への `apply`**・**`global` での丸ごとの配備**・**関門を `mode` で
    分岐させる変異**・**engine から関門の呼びを消す変異**)を実際に仕込んで撃つ。

59. **進捗は状態ではない。走っている環は、証拠を携えて鼓動せねばならない。**
    第53条は「閉じるか、畳むか」の二択を置いた。実測がその二択の**外**を暴いた:

    ```
    $ node graph/conclave.js audit
    🔴 [paradise] sovereign-abode  domains 3/6  🔴 環が閉じぬまま 1483分 無音 [>1440分]
    EXIT=1
    ```

    **門は正しかった。** 走行帳の最後の事象は `dispatch build` のままである。
    だが現実には環は回っていた —— `sovereign-abode` は build 相 1 本を
    work-0〜7 の **8 本**へ割った改革であり、各段が別々の PR として神のマージを待つ。
    四段が既に着地し、証拠が三本倉に在った。**閉じてもおらず、畳んでもおらず、
    見捨てられてもいない走行**が存在したのである。第53条の校正(閉じた走行 5 本の
    事象間隔 173 件)には、**複数の PR に跨がり神のマージを待つ改革**という形が
    含まれていなかった。この形では、一つの相が数日走るのが正常である。

    (a) **欠けていたのは門ではなく口である。** `done` は使えない —— 残る段が在るのに
    相を done にするのは第27条違反。`resume` も違う —— 走者は死んでいない。
    **段が着地したという真の進捗を記す語が機構に無かった。** ゆえに engine は
    `beat`(鼓動)を持つ。鼓動は **記録であって進行ではない**。

    (b) **鼓動は状態を一切変えない。** 相も domain も動かさない。done にも ratified にも
    できない。状態を動かせる鼓動は、第27条の穴を裏口から開け直す。

    (c) **進捗の名乗りには証拠が要る。** 実在し、中身を持つ一つの実物を名指せ
    (0 バイトも空白だけも拒む —— **在ることは資格ではない**: 第54条・第16条)。
    かつ **同じ証拠で二度は鼓動できない**。住所でも中身の sha256 でも拒む ——
    **写して名を変えた証拠は、同じ証拠である。** この錬が無ければ、古い一枚を
    毎日指すだけで第53条の門を永久に黙らせられる。**沈黙を買える口は、門の穴である。**
    同じ理由で、何が着地したかを言わない註釈(空・プレースホルダ)も拒む(第16条)。

    (d) **鼓動は永久の免罪符ではない。** 鼓動もまた一つの事象であり、`ABANDONED_MS` は
    鼓動にも同じく掛かる。一度打てば緑が続くなら、門を消したのと同じである。

    (e) **進んでいると名乗る走行は、黙って緑に落ちない。** 鼓動を持つ走行は
    `audit` の画面で **最後の鼓動が何を携えていたか**(証拠の住所と註釈)を語る。
    免除は記録されて初めて例外である(第54条(c)を鼓動へ向ける)。

    (f) **この条は第53条を緩めない(第57条)。** `ABANDONED_MS` は 24時間のままであり、
    例外リストも `|| true` も無い。鼓動は `audit` の exit を握らない ——
    可否を決めるのは今まで通り `abandoned / unknown / unreadable` の数だけである。
    **門を緩めずに直す道は、門の側ではなく機構の側に口を建てることである。**

    **これを強制する門**: `graph/conclave.js` の `beat` / `beatsOf` /
    `validateBeatNote`(五つの錬は一箇所にだけ住む)と、`runAbandonment` が運ぶ
    `beats` / `lastProof`。回帰は `tests/abandoned-run.test.js` の C 節が、
    偽の証拠・二度指し・写した証拠・走っていない相・空の註釈を**実際に仕込んで**
    撃ち、かつ **逆向き**(鼓動の無い未完の走行は今まで通り赤い)を撃つ。

60. **弱い印で裁く門は、強い印を要求しなければ両方向に誤る。**
    語彙と倉の印という無関係に見える二箇所で、同じ病が同時に発症した。
    *(実測は `reform/route-misfire/` の requirements/review/verify に在る)*

    (a) **弱い印とは、問いより狭い答えしか返さない印である。**
    `git config --get remote.origin.url` は「倉の**中**か」しか答えないのに、
    門はそれを「倉**そのもの**か」として使った。engine の名も同じ ——
    ...
    **強い印とは、問いと同じ広さの答えを返す印である。**
    だが**弱い印を捨ててもならない。弱い印は強い印と組んで初めて働く**(第57条)。

    (d) **門そのものが弱い印で自らの発火を決めてはならない。**
    `if (!fs.existsSync(REAL_VAULT)) skip(...)` — 「対象がたまたまこの機械に在る」は
    弱い印であり、CI に兄弟倉は無いので門は**永久に黙った**(skip は声に出していた)。
    **門は対象が在るのを待つのではなく、自ら建てよ。実在を見る断定は別の門に分けよ ——
    一つの門が「在れば撃つ」と「必ず撃つ」を兼ねてはならない。**

    (e) **印を強めるとき、強めた枝が新たに開く面を測れ。**
    印の強さは二値ではなく**順序**である(`楽園` > `conclave` > `vendor`)。
    最も重い: **強い枝に許した動詞集合が、そのまま新しい弱い印になった** ——
    「改める動詞」は世間の願いにも同じ頻度で現れた。ゆえに
    **印を一つ足したら、足した印そのものに (b) の両方向試験を課せ。
    その面を測らない修理は、病を移しただけである。**

    (f) **読まれて守られない条文は、条文の不足ではなく門の不足である**
    ((e) は既に書かれており、次の相は読んだ上で課さなかった)。ゆえに
    **語彙の表を新設したら、全語について楽園側と世間側の両コーパスを持ち、
    表とコーパスを機械で照合せよ。照合の軸は人が選ぶ ——
    選ばれなかった軸は、機械化しても見えない。**

    (g) **二つの守りが同じ例を守っているなら、変異試験は黙る**
    (紛れ語の表を空にしても器の表が単独で守り、門は一本も鳴らなかった)。ゆえに
    **守りが n 枚在る面には「その一枚だけが守っている例」を n 通り持ち、
    各例について他の守りが立っていないことを先に assert せよ。**

    (h) **同じ印を四度強めても回帰が続くなら、その印は捨てよ。**
    engine の固有名を印にする試みは四度とも新しい回帰を生んだ。
    **取りこぼしを減らせば誤着が増える** —— (b) の定義そのものであり、
...
61. **判定器を直したら、その判定器が奪いうる全ての行き先を数えよ。**
    ある道への誤着を塞ぐ修理は、必ず別の道から奪う。実測: 欠陥Aのため
    `CREATE_RE` に建造動詞を足した結果、熟議の願いが counsel を失った ——
    main 0/150 に対し HEAD 120/150。全門は緑だった。
    *(実測は `reform/route-misfire/reflect.md` §二度目 §2)*

    (a) **門は対角線でなく混同行列で建てよ。** 「A へ着くべきものが A へ着いた」を
    n 件撃っても、「**B へ着くべきものが A へ攫われた**」については一言も述べていない。
    道が n 本なら n×n のセルを持ち、**対角の合計と非対角の各セルが 0** を
    両方 assert せよ。語彙を足せば全セルが再計算され、片側だけ見る修理が不能になる。

    (b) **触れた定数を読む全ての経路を辿れ。** 定数は一つでも読み手は複数居る。
    `CREATE_RE` を広げた者は `isCounsel` の打ち消しを読まなかった。
    **修理の差分が触れた各定数について、それを読む関数を列挙し、各々に探針を持て。**

    (c) **コーパスの冗長は門の沈黙である。** 標識を二つ持つ願いは、語彙の一語が
    死んでも他方が拾う。**語彙の各語につき、その語だけが標識である願いを一件持て。**

    **これを強制する門**: `tests/route-matrix.test.js` —— 6 道×6 道の行列を組み、
    非対角の各セルを名指しで assert する。`W-*` は門自身の骨抜き(恒真 assert・
    コーパスの削り・格子の空化)を見張る。刻んだ教訓が門に届いたかは
    `tests/lesson-export.test.js` が原本(KG)と帳を id の集合で突き合わせる。
...
62. **門の形が、門の盲点を決める。振る舞いだけを撃つ門は、己の作法が隠した層を永久に見ない。**
    ...
    **14 件(23.3%)に対して一言も鳴らなかった**。無音は層に偏っていた ——
    ...
    番兵には**最低限の四つ**を課す —— 緩い条文は最弱の実装を許し、実測でそのすべてが破れた。
    ① **不在(ENOENT)と不能(EISDIR / EACCES / ELOOP)を別の値で名乗れ。** 現物を読む式は
    節の最上位に住むので、**投げれば門が一本も走らない**(実測: ディレクトリを一つ作るだけで
    484 門が全滅した)。読めないは skip ではなく**赤**である —— 不在も不能も通過ではない。
    ② **「前」はその門の直前であって走行の開始ではない。** 読み込み時の一度の観測に固定すれば、
    **その一瞬だけ現物を隠すだけで番兵が走行のあいだ永久に無力になる**(実測: 汚しても全門緑)。
    毎門で測り直し、**一度「無い」と名乗った後に現れたら鳴れ。**
    ③ **汚した門を名で呼べ。** 「誰かが汚した」は名指しではない。
    ④ **外部の正当な営み(追記)と門の汚染(既存行の書き換え)を、分けられるなら形で分けよ。
    分けられないならそう書け** —— 偽の赤は真の赤より有害である(人は偽の赤を学習する)。
    ...
    **これを強制する門**: `tests/paradise.test.js` の gauge 節 —— 実台帳の指紋を毎門で照合する
    ...
    三本とも**壊して鳴ることを別の門が撃つ**(第21条)—— 番兵は
    `gauge(番兵): 実台帳の番兵は本体の例外を飲み込まない` が、並行は
    `gauge(並行): 競合の裁定は純関数であり、門から呼ばれている` が、静的は
    `gauge(静的): 静的の門の射程は外から凍らされている` が、それぞれ**門の外から**
    本文を読んで配線・射程・裁定を凍らせる。**この三本は後から建てられた** ——
    条が「三本とも」と名乗った時、外から撃つ門は番兵の一本しか無く、
    残る二層で無音が二件出た(射程の半減・早期 return の条件潰し)。
    **掟が実体より大きく名乗れば、次の走行はその嘘を土台にする。** 実体を掟に追いつかせよ。
    **そして無音率は次の走行で測り直される** ——
    条を足したという主張も、次の変異走行の数が前より良くなって初めて証明される(第38条)。

63. **同じ入力の走行を二度撃つな。ただし畳んだことを機械が名乗らない畳みは、測定の放棄である。**
    実測された欠陥: 一回の CI のあいだに、**入力が一字も違わない同じ自己診断が繰り返し撃たれ**、
    同じ成果物が繰り返し検められていた。走行は正しく、緑も正しく、ただ**同じ答えを
    何度も買っていた**。第55条は「同じ観測を二度刻んだ帳は騒音である」と台帳を律したが、
    **刻む前の走行そのものは誰も律していなかった** —— 台帳の冪等性は、重複した実行を
    綺麗に畳んで見せることで、むしろ重複を見えなくする。

    だが畳みは第37条(不在は通過ではない)の**最も自然な裏口**でもある。畳まれた走行は
    「走らなかった」のに「緑」を名乗る。ゆえに畳みは、節約の機構である前に、
    **名乗りの機構でなければならない。**

    (a) **鍵は入力の中身から採る。** 「同じ走行か」を時刻・順番・呼び出し側の主張で決めるな。
    入力の内容ハッシュだけが答える。**鍵の住処は一つである** —— 二つ在れば鍵が二通りに割れ、
    片方が畳んで片方が畳まない日が来る(第48条・第58条)。

    (b) **畳んだことは毎回 stdout で名乗れ。** `Executed E out of N (R reused, key=…)` ——
    **撃った数・撃つはずだった数・再利用した数**が閉じていること。
    名乗らない畳みは、速くなったのか壊れたのかを**誰にも区別させない**。
    畳みの効果を主張するなら前後の数で証明せよ(第38条)。

    (c) **畳まなかった理由は閉じた語彙で名指せ。** 語彙を開けば `bail=whatever` が生まれ、
    機械は「畳む必要が無かった」と「畳む機構が壊れていた」を同じ文で語る。
    とりわけ **「領収書が無い」と「台帳が読めない」は別の語でなければならない** ——
    読めないは skip ではなく赤である(第62条(b)①)。語彙外の bail を組み立てようとしたら、
    黙って通さずその場で倒れよ。

    (d) **領収書の有効範囲を宣言し、それを超えて持ち越すな。** 走行をまたいで領収書が
    生き延びれば、今日の門が**昨日の緑**を名乗る。畳みの台帳は一回の走行の中でのみ有効であり、
    宣言外の状態に依る走行(`=global`)は**構造的に畳まない** —— 畳めるかを賢く判定するのではなく、
    判定できないものは畳まない。

    (e) **出口は常に開けておけ。** 畳みを疑った者が、一つの旗(`--no-fold` /
    `PARADISE_NO_FOLD=1`)で畳まぬ走行を撃てること。そして**切ったことも名乗ること**
    (`bail=disabled`)。疑いを晴らす手段の無い最適化は、最適化ではなく信仰である。

    (f) **畳みは門を減らす免罪符ではない。** 畳んだ後も門の総数は一本も減っていないことを、
    数え直す機構が言うこと(第22条)。「速くした」の代償に門を間引いたなら、それは畳みではなく削除である。
