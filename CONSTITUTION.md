...
60. **弱い印で裁く門は、強い印を要求しなければ両方向に誤る。**
    ...
    **これを強制する門**: `graph/forge.js` の `ENGINE_NAMES_STRONG` /
    ...
    三度目の差し戻しが (f) を当て直した結果、条文の側に欠けは見つからなかった —
    (f) は「**印を一つ足したら、足した印そのものに (b) の両方向試験を課せ**」と
    既に書いており、`MEND_RE` を足した相はそれを**読んだ上で**課さなかった。
    **読まれて守られない条文は、条文の不足ではなく門の不足である。**
    ゆえに (f) に一段を継ぐ:

    **語彙の表を新設したら、その表の全語について「楽園側」と「世間側」の
    両方のコーパスを持て。そして表とコーパスを機械で照合せよ。**
    `MEND_RE` を足した相は楽園側を 12 件撃ち、世間側を **0 件**撃った。
    「強い名 26 語を網羅せよ」という機械照合の門は既に在ったが、
    **その門は「名の軸」だけを照合し、「動詞の軸」を照合しなかった。**
    **機械照合の門は、照合する軸を人が選ぶ。選ばれなかった軸は、機械化しても見えない。**
    ゆえに **表を足した者が、その表を軸とする照合の門も同時に足す**
    (`MEND_WORLDLY_EVERY_VERB` が `MEND_RE` の源と突き合わされ、
    語を足してコーパスを足さなければ赤くなる)。

    (h) **二つの守りが同じ例を守っているなら、変異試験は黙る。**
    三度目の故障注入が実測した — `門` の紛れ語を撃つ門は
    `gauge 専門店の壊れた棚を直す在庫アプリ` のように**器の名を持つ例ばかり**を並べていた。
    そのため `ABSTRACT_FALSE_FRIENDS`(紛れ語の表)を**丸ごと空にしても**、
    `WORLDLY_VESSEL_RE`(世間の器の表)が単独で守り、門は**一本も鳴らなかった**。
    これは (e) と同じ病が**門の例の側**に出た形である。
    ゆえに: **守りが n 枚在る面には、「その一枚だけが守っている例」を n 通り持て。**
    各例について、**他の守りが立っていないこと**を先に assert せよ —
    前提が崩れた門は、緑を騙る門である(第37条)。
## The Verdict Law
