...
| # | 要件 | 数値・判定 |
|---|---|---|
| **NFR-01** | 全走への上乗せ | 引数なし全走の所要時間は **前(358.8 秒)の +1.0% 以内 = 362.4 秒以下**。設計上の目標は **+0.5 秒以下**(`test()` に条件分岐 1 個を足すのみ) |
...
入力: `time node tests/paradise.test.js > /dev/null` を **background** で 1 回
期待: 実時間 **362.4 秒以下**(前値 358.8 秒 + 1.0%)、最終行 `451 passed, 0 failed`、`EXIT=0`
壊し方: `test()` ごとに正規表現をコンパイルし直す実装(451 回)は
