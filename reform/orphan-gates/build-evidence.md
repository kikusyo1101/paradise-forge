...
## §6 門自身の欠陥を、門を撃つことで見つけた

メタ門を建てた後、**「将来足される門」の側から**撃ち直した —— 孤児を仕込む逆方向
だけでなく、**正しく呼ばれている門を正しく認めるか**も証さねば、門は壁になる。

複製の倉で三段に分けた:

```console
$ cp -r . "$T/umb2" && cd "$T/umb2"
$ printf "console.log('ok');\n" > tests/umbrella-demo.test.js

--- (1) 誰も呼ばない → 孤児のはず ---
      tests/umbrella-demo.test.js — 建てられているが、CI も paradise.test.js も走らせない

--- (2) paradise.test.js の *註釈* に名前だけ書く → 依然 孤児のはず (第16条) ---
      tests/umbrella-demo.test.js — 建てられているが、CI も paradise.test.js も走らせない

--- (3) 実際に走らせるコードを書く → 呼ばれていると認めるはず ---
  ✓ 門 20 本すべてに走らせる者が居る (第44条)
EXIT=0
```

**(3) は最初、赤かった。** これは門自身の欠陥だった。統べる試験が兄弟を呼ぶとき、

```js
execFileSync('node', [path.join(__dirname, 'x.test.js')])
```

と書くのが最も自然である —— 自分と同じ部屋に住むので `tests` という綴りを
**一度も書かない**。この形を数えていなかったため、門は**実際に呼ばれている門を
孤児と報告していた**。

**呼んでいるのに孤児と言うのは、孤児を見逃すより悪い誤審である。**
`wiring.js` は同じ教訓を器物で既に学んでいた(`NAME_RES` の註釈:
「門が呼び方の**綴り**しか見なければ、生きた engine に死亡宣告を下す」)。
その教訓が門の新しい面に届いていなかったのである。`__dirname` 結合の形を足して
撃ち直したのが上の実出力である。

**(2) が孤児のままであることが同じくらい重要である** —— 註釈に名を書くだけで
門が「呼ばれている」と認めてしまえば、§3.3 の病がそのまま再発する。
