...
# 【四周目】verify — 出荷に足るかを裁く

> **これが最後の周である。** construction は reworks=3(MAX_DOMAIN_REWORK=3)。
> ゆえに本節の仕事は差し戻しではなく、**裁定**である。
...
```
                                    main         HEAD        向き
────────────────────────────────────────────────────────────────
誤着(蓄積コーパス 106 件)           8           0        -8  改善
誤着(四周目の新しい 60 件)          15           4       -11  改善
  うち本走行が新たに壊したもの        —           0         ±0
門(counsel の断定)                 51         210      +159  増
門(abandoned-run の断定)           20          33       +13  増
門(ファイル数)                      19          19        ±0
全走(神の機械)              469 passed  471 passed        +2
             0 failed 2 skipped   0 failed 0 skipped
残った正規表現(攻撃面)              9 本        9 本        ±0
公開 export(誤用の面)             21          21        ±0  ※c6b5ba9 比では 35→21
────────────────────────────────────────────────────────────────
```

**すべての軸で main 以上であり、悪化した軸は一つも無い。**
欠陥A は治り、欠陥B は治り、欠陥C 以降は**実測とコーパスを添えて申し送られた**。

---

## V4-1. AC 全 46 件の**五区分**格付け

区分: **達成** / **未達** / **検められない** / **門が無い** / **射程外(次の走行へ)**

> 四度目の build(`ENGINE_NAMES` の撤去)で、**engine の固有名を印にすることを
> 前提とした AC は実装が保証しない面になった。** それらを **射程外** と格付ける ——
> 「未達」ではない。**教主の裁定により意図して捨てた面**であり、
> 第60条(h) に従い実測とコーパスを添えて `requirements.md` §8 へ申し送られている。

| AC | 格付 | 証拠(実機の生出力) |
|---|---|---|
| AC-01 | **達成** | `楽園の自己診断に絞り込みの口を設ける` → reform |
| AC-02 | **達成** | `門に監査の一段を足す` → reform |
| AC-03 | **達成** | ``CI に `ledger --audit` を追加する`` → standard(≠counsel) |
| AC-04 | **達成** | `健康診断アプリが欲しい` → full(≠counsel) |
| AC-05 | **射程外** | `gauge に fingerprint を確かめる口を設ける` → standard。requirements §8.1 が題そのものとして申し送る |
| AC-06 | **達成** | AC-04 と同一。full |
| AC-07 | **達成** | `楽園の位階の相関図を作れ` → cartography |
| AC-08 | **達成** | `楽園のエンジンを監査してほしい` → counsel |
| AC-09 | **達成** | `台帳の毒を直す` → quick |
| AC-10 | **達成** | 判定表の緑行数 **47**(37 行以上を要す)/ counsel EXIT=0 |
| AC-11 | **達成** | `isCounsel('比較表がほしい')=true` / `isCounsel('タイマーが欲しい')=false` / 門 ✓ |
| AC-12 | **達成** | `typeof chooseScale(...)==='string'` = true / 門 ✓ |
| AC-13 | **達成** | `const COUNSEL_JA` / `const COUNSEL_EN` が forge.js に在る = true / 門 ✓ |
| AC-14 | **達成** | ✓ 日本語に \b を使うと語彙は死ぬ(既存バグの回帰固定) |
| AC-15 | **検められない** | `conclave.js audit` **EXIT=1** —— だが名指されたのは **`sovereign-abode` (domains 3/6 / 5115分 無音)** であり、**main でも同じ EXIT=1・同じ走行**。本走行と無関係の既存状態。`route-misfire` 自身は `▶ domains 4/6` で健全 |
| AC-16 | **門が無い** | 全走は実際に撃った: **Paradise self-test: 471 passed, 0 failed** —— だが「6 分を払え」を強いる門は存在しない(grep 0 件) |
| AC-17 | **達成** | ✓ admit の裁定は chooseScale と一致する (AC-17 / paradise.test.js:8566 の主張) |
| AC-18 | **達成** | 仮倉 → EXIT=0 |
| AC-19 | **達成** | 実測: `· 走行帳の流出は検めなかった — …\\fakevault は創造物の倉ではない (目印 … も git remote … も無い / source=env)`。門も在る(`abandoned-run.test.js:409-410` が `/検めなかった/` と `/創造物の倉ではない/` を撃つ)。**三周目の「門が無い」から昇格** |
| AC-20 | **達成** | 本物の倉 → EXIT=0 / `✓ …・reform 走行帳の流出なし (検めた倉: …\\paradise-creations)` / 門 B-10 ✓ |
| AC-21 | **達成** | ✓ B-1 [故障注入]: 創造物の倉に居る reform の走行帳を名指しする |
| AC-22 | **達成** | 四周目が**替え玉の倉を建てて**実測: 根=true / 子=false / 孫=false / `.git`=false。門 B-12(替え玉) + B-12b(実物)。**B-12 は本物の倉に依存しないので CI でも鳴る** → **三周目の「門が無い」から昇格** |
| AC-23 | **達成** | 名でも指され方でも裁かない(`some-vault-name` の替え玉が true / `paradise-creations` という名の仮倉は false) |
| AC-24 | **達成** | 目印 `.paradise-creations` だけの倉で `isCreationsVault=true` / 門 B-11 ✓ |
| AC-25 | **達成** | `resolve()` のキー: HEAD=`exists,legacy,root,source,vault` / main=`exists,legacy,root,source` → **消えたキー: なし**(足しただけ) |
| AC-26 | **達成** | **四周目が PATH から git を消して実機で撃った**: `isCreationsVault=false` / **例外を投げない**。門 B-14(注入面) → **三周目の「門が無い」から昇格** |
| AC-27 | **達成** | `wiring.js check` EXIT=0 / ✓ 門 19 本すべてに走らせる者が居る (第44条) |
| AC-28 | **達成** | `workspace.js check` EXIT=0 / 住所の直書きなし |
| AC-29 | **達成** | `census.js fix` = **nothing to fix** / `census.js check` **EXIT=0** / `✓ every number the paradise claims about itself is true`。CI も `.github/workflows` から呼んでいる → **三周目の「門が無い」から昇格** |
| AC-30 | **達成** | CLI⇔lib 3 件で一致: `門に監査の一段を足す` reform/reform・`健康診断アプリが欲しい` full/full・`楽園のエンジンを監査してほしい` counsel/counsel。**割れ 0**(三周目の「CLI 拒否 2 件」は engine 固有名の願いで、撤去により CLI が受け付ける形になった) |
| AC-31 | **達成** | 世間の願いが reform へ攫われた= **0**。106 件の蓄積コーパス + 四周目の 60 件で確認 |
| AC-32 | **射程外** | `vendor に口を設ける` → standard。engine 固有名を印にしないので保証しない面。§8.1 が申し送る |
| AC-33 | **射程外** | 冠詞の枝(`DETERMINER_LOOKBEHIND`)そのものが撤去(`undefined`)。**面は生きている**(5/5 が非 reform)が、それを撃つ枝が無い。§8.8.5 |
| AC-34 | **射程外** | `ENGINE_NAMES` が `undefined`。網羅の門は削除され、代替が `graph/*.js` の実在と照合する形で生存 |
| AC-35 | **門が無い** | 変異を自動で当てる門は無い(人が手で撃つ)。**四周目は M1/M2 を手で撃ち、Q3-1/Q3-2 が両方鳴ることを実測した**(review R4-6) |
| AC-36 | **達成** | 教主の 10 件 reform 誤着= **0/10**(門 ✓ 10 本すべて緑) |
| AC-37 | **達成** | engine 名 **26 語すべて**の世間の願いが非 reform。門「強い名のコーパスが engine 名を過不足なく覆っている」が `graph/*.js` と照合 ✓ |
| AC-38 | **達成** | **抽象名の形で**生存: `楽園の抽象名は改変の動詞を伴えば今まで通り楽園を名指す` ✓。engine 固有名の面は射程外 |
| AC-39 | **射程外** | 枝が 1 本になった(`REFORM_STRONG_RE`=`undefined`)。「二条件がそれぞれ単独で効く」を撃つ対象が無い |
| AC-40 | **達成** | 抽象名 12 語すべてで `isReformSubject`=true(無条件のまま)。門「REFORM_RE は main の抽象名を過不足なく持つ」✓ |
| AC-41 | **射程外** | `MEND_RE`=`undefined`。**34 語のコーパスは `MEND_WORLDLY_EVERY_VERB` として残っている** |
| AC-42 | **門が無い** | 11 変異を自動で当てる門は無い。四周目の M1 が AC-36/37/43 系を一挙に赤くすることを実測(64 failed) |
| AC-43 | **達成** | counsel EXIT=0 / 34 語 x engine 名 x 世間の器のコーパスが全て緑 / **210 passed, 0 failed** |
| AC-44 | **達成** | 「engine 名 + 改める動詞だけでは楽園を名乗らない」に**改名して生存** ✓ |
| AC-45 | **達成** | **三周目の「未達」から昇格。** 器を持つ本物の門 4/4 が **reform**(三周目は 4/4 が quick へ落ちていた = Q3-1)。紛れ語の世間の願い 8 件は **8/8 が main と同値**(悪化なし)。門「「門」の紛れ語は main と同じ振る舞いである」✓ |
| AC-46 | **達成** | 四周目の故障注入 **M1 → 64 failed / M2 → 1 failed**。**新設 2 門とも鳴った。黙った門ゼロ** |

### 五区分の集計と、周ごとの推移

```
        達成  未達  検められない  門が無い  射程外
一周目   28    0        0          7       —
二周目   32    1        0          8       —
三周目   37    1        0          8       —
四周目   34    0        1          4       7
...
**読み方:**
- **未達 0 件**(三周目の 1 件 = AC-45 が**達成へ昇格**)。
- **門が無い が 8 → 4 に減った**。AC-19 / AC-22 / AC-26 / AC-29 が四周目の実測で**達成へ昇格**した ——
  三周目は「撃っていない」を「門が無い」と記していたが、**撃てば緑だった**。第37条の正しい扱いである。
- **射程外 7 件**(AC-05 / 32 / 33 / 34 / 39 / 41 と、AC-38 の engine 固有名の面)。
  **これは失敗ではない。** 教主が四度の実測を根拠に**意図して捨てた面**であり、
  第60条(h) が要求する「実測と門のコーパスを添えた申し送り」は §8.1〜8.5 に在る。
- **検められない 1 件**(AC-15)。`conclave.js audit` が **main でも同じ EXIT=1** を出す ——
  名指されたのは `sovereign-abode` であって本走行ではない。**本走行の責ではない。**
- **達成が 37 → 34 に減ったのは、7 件が射程外へ移ったからである。**
  同じ母数で比べれば **(34 達成 + 7 射程外) = 41 件が「意図した通りの状態」** であり、
  三周目の (37 達成) より**広い**。

---

## V4-2. 第38条の前後 —— main と HEAD で門の数と誤着数
...
```
                          main      HEAD     差
門のファイル数             19        19      ±0
counsel の断定             51       210    +159
abandoned-run の断定       20        33     +13
全走(神の機械)      469 passed  471 passed   +2
                 0 failed     0 failed
                 2 skipped    0 skipped
```

**一本も減らしていない。** 削除された 18 門は**依存する機構が消えたもの**であり、
代替が同じ面を守る(review R4-4)。

### 誤着数 —— **蓄積コーパス 106 件を両方で撃った**

```
軸                             件数    main誤    HEAD誤  増減
──────────────────────────────────────────────────────────────
A 世間×engine名 (26語網羅)        26        0        0  ±0
B 世間×限定詞                    12        0        0  ±0
C 世間×建造×器                    8        0        0  ±0
D 楽園×器 (37語)                 37        0        0  ±0
E 欠陥A 二つの顔                  14        8        0  -8 (改善)
F 楽園×抽象名                     9        0        0  ±0
──────────────────────────────────────────────────────────────
合計                          106        8        0  -8 (改善)
...
HEAD に残る NG 4 件は**すべて main でも NG** であり、
§8.8.1 / §8.8.2 / §8.8.3 として申し送った。

---

## V4-3. 故障注入の結果 —— **黙った門ゼロ**

| 変異 | 内容 | counsel の結果 | Q3-1 | Q3-2 |
|---|---|---|---|---|
| 基準 | 無改変 | 210 passed, 0 failed | ✓ | ✓ |
| **M1** | `REFORM_RE` に engine 固有名 12 語を足す(四度の回帰の再演) | **146 passed, 64 failed** | ✓(緑のまま) | **✗ 赤くなった** |
| **M2** | 抽象名の枝に `WORLDLY_VESSEL_RE` の守りを入れる(三度目の build の形) | **209 passed, 1 failed** | **✗ 赤くなった** | ✓(緑のまま) |

**神官が「実装がその面を持たないから緑」と自ら疑った新設 2 門は、両方とも生きた門であった。**

⚠️ **二本は互いに逆向きなので、一つの変異では片方しか鳴らない。**
M1 は「印を広げる」変異なので Q3-2(広げすぎを咎める門)が鳴る。
M2 は「守りを足す」変異なので Q3-1(狭めすぎを咎める門)が鳴る。
**n 個の門には n 個の変異を要する。**
「一つの変異で両方が鳴らない」を第60条(d) の黙った門と誤読してはならない。

**名指しで報告すべき黙った門: **なし**。**

変異後は両方とも `git checkout -- graph/forge.js` で復旧し、`210 passed, 0 failed` を再確認した。
**本物のリポジトリは一度も汚していない**(複製 `$LOCALAPPDATA/Temp/pd_inject` で実施)。

---

## V4-4. 出荷の可否の裁定 —— **数で答える**

> 問い: **この走行は欠陥AとBを治し、欠陥C以降は撤去して申し送った。
> それで main より良いと言えるか。**

### 良いと言える。根拠は五つ。

**1. 誤着が減った。増えたものは一つも無い。**
```
蓄積コーパス 106 件:  main 8 誤着 → HEAD 0 誤着   (-8)
新しい 60 件:         main 15 NG → HEAD 4 NG      (-11)
新たに壊したもの:      0 件
```
...
```
counsel:        51 → 210  (+159)
abandoned-run:  20 →  33  (+13)
門のファイル数: 19 →  19  (±0 / 孤児ゼロ)
```
削除された 18 門はすべて**依存する機構が消えたもの**で、
「今の形でも緑になる」2 本も代替が同じ面を守る(review R4-4)。**正味の損失ゼロ。**

**3. 治った欠陥は実測で生きている。**
```
欠陥A: 紛れ語 67/67 / 本旨 NG=0/18 / isCounsel の四本の帰り道すべて
欠陥B: 仮倉 skip / 毒 3 種で EXIT=1 / HIGH-1 (根 true・子孫 false) / V-1 の 11 形すべて
```

**4. 捨てた面は隠していない。**
第60条(h) が要求する形で、**実測(§8.2 の四度の回帰 / §8.3 の三案の数)と
門のコーパス 8 種(174 件 / 実在を機械で確認)**を添えて申し送った。
**捨てた面を撃つ門は残っていない** —— 残せば第60条(d) の黙った門になるからである。

**5. 安全になった。**
```
ReDoS 危険:    0 件 / 10,000 字で最大 0.581 ms
攻撃面:        正規表現 12 → 9 本 / export 35 → 21 / 枝 3 → 1
死んだ参照:    0 件(14 名すべて undefined / 残る出現は註のみ)
```

### 反証の検討 —— 「悪くなった」と言える軸は在るか

| 候補 | 検め | 裁定 |
|---|---|---|
| engine 固有名の願いが reform に着かなくなった | **main でも着いていない**(`gauge に…口を設ける` は main=standard / HEAD=standard) | **悪化ではない**(現状維持) |
| 門を 18 本消した | 代替が同じ面を守る / 断定は +172 | **悪化ではない** |
| 第60条が 101 → 53 行に縮んだ | 教訓 (a)〜(h) は一つも消えていない。消えたのは撤去された機構の記述 | **悪化ではない** |
| `conclave.js audit` が EXIT=1 | **main でも EXIT=1**(`sovereign-abode`) | **本走行の責ではない** |
| README の数がずれた | `census.js fix` = **nothing to fix** | **ずれていない** |
...
### 7. counsel / abandoned-run 0 failed / wiring EXIT=0 / codex EXIT=0
...
### 8. paradise 全走の実数(background)

```
$ node tests/paradise.test.js          # HEAD (c43e093)
Paradise self-test: 471 passed, 0 failed

$ node tests/paradise.test.js          # main (c216014) の clone
Paradise self-test: 469 passed, 0 failed, 2 skipped
MAIN_FULL_EXIT=0
```

### 9. census fix → check EXIT=0(background)
...
## V4-6. 【四周目】自分が見ていない項目(第37条)

1. **CI(tribunal.yml)を実際に走らせていない。** 掟により push していない。
   **main の全走は神の機械で 2 件 skip した** —— HEAD は 0 skip だが、
   **CI の裸の機械で何件 skip するかは見ていない**(README は 10 skip と記す)。
2. **`sovereign-abode` の走行を閉じていない。** `conclave.js audit` の EXIT=1 の原因だが
   **main でも同じであり、本走行の射程外**。教主の裁きを待つ。
3. **変異は 2 種類。** 三周目の 15 変異を撃ち直していない(対象の多くが撤去された)。
4. **AC-16 / AC-35 / AC-42 は「門が無い」のまま。** 全走を強いる門・変異を自動で当てる門は
   本走行では建てていない(射程外)。
5. **`DOC_STRONG_RE` を表として直に撃てていない**(export されていない / security §S4-5)。
6. **日本語・英語以外の言語**を一件も撃っていない。
7. **兄弟倉(paradise-creations)の中身を検めていない** —— `workspace.js check` の
   裁きが緑であることをもって代えた。
