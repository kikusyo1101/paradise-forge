...
| 三代(rework2) | **抽象名 / 強い固有名 / 弱い名の三枝**。枝 2 に `BUILD_RE ∪ MEND_RE` | **Q2-1** —— `MEND_RE` × 強い名 の**世間の願い 32/32** が攫われた(main 0/32) |
| 四代(本修理) | **四枝**。枝 2 を建造と改修に割り、**改修の側にだけ `mendsParadise` を課す** | — |

**四代の形**:

```
isReformSubject(d):
  1.  namesParadiseAbstractly(d)                                → true   (無条件。ただし一字の `門`/`gate` は紛れ語を退ける)
  2.  REFORM_STRONG_RE(d) && BUILD_RE(d)                        → true
  2'. REFORM_STRONG_RE(d) && MEND_RE(d) && mendsParadise(d)     → true
  3.  REFORM_WEAK_RE(d)   && BUILD_RE(d)

mendsParadise(d) = !WORLDLY_VESSEL_RE(d) && STRONG_BOUND_RE(d)
```

**枝 1 が無条件でよい理由**: 抽象名(楽園/engine/憲法/走行帳/自己診断)は
**楽園以外のものを指さない**。世間の願い文に「楽園の」とは書かれない。
ここに動詞を課せば `楽園はどうあるべきか` 系が死ぬ(AC-40 が撃つ)。
**ただし `門` は唯一の一字の名であり紛れ語を持つ**(`専門店`/`門前町`/`名門`/`部門別`)——
`ABSTRACT_FALSE_FRIENDS` が `PRODUCT_FALSE_FRIENDS` と同じ作法で守る(AC-45)。
これは **main 由来の病**であり、三代までは誰も測っていなかった。

---

#### 1.5.1.1 【Q2-4 の訂正】非対称の根拠は「証拠の強さ」ではない

> ⚠️ **三代の設計は次のように書いていた。この推論は実測に反する。訂正する。**
>
> > 強い名は**楽園固有性の証拠として強い**ので、許す動詞集合も広くてよい(`BUILD_RE` ∪ `MEND_RE`)。
> > **証拠の強さが許す範囲を決める。**

**この根拠が真なら、枝 2 に `MEND_RE` を許しても枝 3 より安全なはずである。実測は逆だった:**

| 枝 | `MEND_RE` を許した時の壊れ方 | 出典 |
|---|---|---|
| 枝 2(強い名) | **32/32** が世間の願いを攫う | quality 二周目 §1(教主も 9/10 を実測) |
| 枝 3(弱い名) | **8/12** が世間の願いを攫う | quality 二周目 §2.1 |

**強い名の方が酷く壊れる。** 理由は第60条(a) が既に書いていた ——
`gauge`(計器)/ `abode`(住居)/ `critic`(批評家)/ `forge`(鍛冶場)/ `clergy`(聖職者)は
**全て世間で日常的に使われる普通名詞**である。「強い名」という呼称は
**楽園の内部での曖昧さの序列**であって、**世間の願い文に対する証拠の強さではない**。

**正しい根拠はこうである:**

> **どの枝にも `MEND_RE` は弱い印である。**(第60条(f): 強い枝に許した動詞集合が、
> そのまま新しい弱い印になった。)
> **枝 2 だけが `MEND_RE` を許されるのは、枝 2 にだけ「守るべき正例」が在るからである** ——
> `conclave の毒を除く` は `BUILD_RE` を一語も持たない、紛れもない楽園の改革である。
> 枝 3 には対応する正例が無い(弱い名は世間の語そのものなので、
> `vendor の請求書テンプレを直す` は楽園の改革ではない)。
> **そして「許す」の代償として、枝 2' には動詞ではない別の印**(`mendsParadise`)**を課す。**
> すなわち **非対称は「証拠が強いから広く許す」ではなく
> 「正例を守るために許し、そのぶん別の印で締める」**である。

**`mendsParadise` の二条件が両方要る理由**(第60条(d): 弱い印は強い印と組んで初めて働く)。
本相が全コーパス 202 件で実測した:

| 案 | 形 | 全コーパスの誤着 |
|---|---|---|
| 修理前(三代) | 枝 2 に `BUILD_RE ∪ MEND_RE` | **78/202** 🔴 |
| (i) `MEND_RE` を枝 2 から外す | 枝 2 も `BUILD_RE` のみ | 36/202(楽園の改修が大量に死ぬ) |
| (iii) 世間の器の不在のみ | `!WORLDLY_VESSEL_RE` | 13/202 |
| (iv) 助詞の結びのみ | `STRONG_BOUND_RE` | 11/202 |
| **採用: 両方** | `!WORLDLY_VESSEL_RE && STRONG_BOUND_RE` | **9/202** → 門の紛れ語も塞いで **2/202** ✓ |
| (参考)main | — | 51/202 |

**`STRONG_BOUND_RE` が日本語にしか立たない**のは測って選んだ非対称である ——
英語に助詞は無い。英語側は `WORLDLY_VESSEL_RE` と限定詞の除外(`the gate`)が受け持つ。

**`WORLDLY_VESSEL_RE` の禁則**: `PRODUCT_RE` と**別の表**である。
`PRODUCT_RE` に在る `口`/`門`/`相`/`フラグ`/`一段` は**楽園の器官の名でもある**ので、
一語でも入れれば `門に監査の一段を足す` が死ぬ。`ダッシュボード`/`dashboard` も同様
(楽園自身が `dashboard/index.html` を持つ)。**表を直に撃つ門**(AC-43)がこれを守る。

**`MEND_RE` が要る理由**: `conclave の毒を除く` は `BUILD_RE` を一語も持たない
(「除く」は建造ではない)。強い名に `BUILD_RE` だけを課せば、
楽園の最も自然な改修の願い(**壊れた物を除く**)が全て死ぬ(実測 (i) = 36/202)。
