#!/usr/bin/env node
/**
 * design 相の受け入れコーパス **101 件** で 6×6 混同行列を撃つ。
 *   = findings 3.2 の 98 件 − 残債 2 件(PARA-10/PARA-11) + 「サイト語 × 部分機能」5 件
 * ⚠️ 判定器を一行も変えない。PROBE_FORGE で写しの forge.js を指せる。
 */
'use strict';
...
// ── standard (22 = 15 + PARA-9 の 2 + サイト語×部分機能 5) ──
...
// ★ specify 相 §2.3 の実測 —— 素の C1 が 5/5 攫った的(第21条: 撃たない的で緑を名乗るな)
...
// ── full (16 = 8 gate + PARA-7 の 7 + PARA-9 の 1) ──
...
// ⚠️ 残債へ移した 2 件(specify §3.4 の裁定):
//    ・一門の家系図を作れるアプリが欲しい  (PARA-10: REFORM_RE の弱い印「門」)
//    ・相関図を描けるアプリを作って        (PARA-11: PRODUCT_FALSE_FRIENDS の文全体無効化)
...
console.log(`判定: ${diag === CORPUS.length && bad === 0 ? '合格 (対角 101/101 かつ 非対角 0)' : '不合格'}`);
process.exitCode = 0;
