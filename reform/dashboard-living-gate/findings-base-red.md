...
### ただしこの赤は**不定である** — 3回目の走行では消えた

```
$ node tests/paradise.test.js 2>&1 | grep -E "✗|failed:|Error:"
(出力なし = 赤ゼロ)
```

**そして赤が消えても漏れは続いている。** 同時刻の実測:

```
$ ls $TEMP | grep -c "archify-visual-check-profile"
519          ← 30分前は 483。走行のたびに増え続けている
...
**これが最も危険な形の欠陥である。** 症状(赤)は不定に出たり消えたりするが、
原因(プロファイルと Chrome の漏れ)は**単調に悪化し続けている**。
緑を見て「直った」と報告すれば、それは第16条違反 —「判定不能は緑ではない」。
ここでは更に悪く、**判定が緑でも欠陥は在る**。
...
## 漏れを1回の走行で実測した（主張ではなく差分）

```
$ node prove-leak.js
BEFORE {"profiles":527,"chrome":10}
AFTER  {"profiles":529,"chrome":10}
LEAK   {"profiles":2,"chrome":0}
🔴 検器1回で一時プロファイルが漏れた — close() が呼ばれていない証拠
```

**検器を1回走らせるだけでプロファイルが +2 個**。走行のたびに単調に増える。
`atlas.js check` は6主題を回すので、門を1回通すだけで十数個が積まれる計算になる。
これが 483 → 519 → 529 という増加の正体である。

> 測り方: `motion-probe.mjs` を実在する図に対して1回だけ実行し、
> `$TEMP/archify-visual-check-profile-*` の個数を前後で数えた。
> Chrome プロセス数は 10 で変わらず(既存の漏れが生き残っているため)。
...
| B-2 | 検器が一時プロファイルを残さない | 検器を1回走らせる前後で `ls $TEMP \| grep -c archify-visual-check-profile` が **増えない**。実測ベースライン: 現在 519 個 |
| B-3 | フル自己診断が緑で完走する | `node tests/paradise.test.js` が `0 failed` |
| B-4 | 残骸で落ちる門を回帰試験が捕らえる | 既存の第21条テストが、B-1 を戻すと**赤くなる**ことを prove 相で確認する |
| B-5 | **漏れそのものを門が数える** | 新規テスト: 検器を1回走らせ、前後のプロファイル数の差が 0 であることを assert する。**症状(不定の赤)ではなく原因(漏れ)を裁く門**でなければ、緑が嘘をつき続ける |

> **B-5 が本質である。** B-3(自己診断が緑)を受入基準にしてはならない —
> この赤は不定であり、**漏れが 519 個まで悪化した状態でも緑を出した**。
