...
test('D-3(故障注入): report にパスを渡すと total=0 になり、防御が spawn=null を返す', () => {
  const r0 = snap.runs[0];
  assert.ok(r0, 'run が 1 件も無い');
  // 誤った呼び方 —— **例外を投げない。静かに緑を返す**
  const wrong = spawnTrace.report(r0.path);
  ...
  const errors = [];
  const guarded = pulse.readSpawn(r0.path, r0.name, errors);
  ...
  // 正しい呼び方なら測れる
  const run = JSON.parse(fs.readFileSync(r0.path, 'utf8'));
  const good = pulse.readSpawn(run, r0.name, []);
